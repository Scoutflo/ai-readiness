...
| SIG-070 | ClickHouse health | Metric-cardinality health — no metric carries an unbounded/identifier label (e.g. `trace_id`, `user.id`, `url.full`) or an accumulating one (`k8s.pod.uid`), and no bounded label exceeds a value-count ceiling while unused by any dashboard/alert; series count is the billable + query-cost driver, and the fix is `metricstransform aggregate_labels` (merge series), never dropping an Infra/APM-page dependency | high |
...
for _t in traces metrics logs; do echo "== $_t =="; sig_get "/api/v1/settings/ttl?type=$_t" | jq '.'; done   # v0.138: the ?type= param is REQUIRED (a bare call returns 400 "type param cannot be empty")
...
## 6c. Metric cardinality health (SIG-070)

High-cardinality metric labels are the quiet killer of a SigNoz/ClickHouse metrics store: every distinct label-value combination is a separate time series, and series count drives both ingestion cost (samples are billable) and query latency. A single unbounded label (a request id, a full URL) on one busy metric can explode a store from thousands to millions of series. SIG-070 reads distinct label-value counts for the top-N metrics by sample volume from the metrics store (discover the actual table/columns live — `signoz_metrics` on current builds; **never hardcode-assert a table/column, confirm it against the instance**, mirroring the SIG-030/060 discovery style) and classifies each label. This is a ClickHouse-lane check: when the ClickHouse read path is not configured/reachable it is `not-in-scope` and the category renormalizes (the SIG-030/060/061 path).

Classify each label against these profiles:

- **UNBOUNDED — flag regardless of current count** (the problem is trajectory, not today's number): `url.full`, `http.url`, `http.target`, `url.path`, `url.query`, `db.query.text`, `db.statement`, `net.peer.port`/`client.port` (ephemeral 32768–60999), `exception.stacktrace`/`exception.message`, `process.pid`/`process.command_line`/`process.command_args`, `thread.id`, `messaging.kafka.message_key`, `cloud.resource_id`/`aws.lambda.invoked_arn`, `faas.instance`/`faas.id`.
- **IDENTIFIER — never valid as a metric label** (immediate red flag): `span.id`, `trace.id`/`trace_id`, `request.id`, `session.id`, `user.id`/`enduser.id`, `messaging.message_id`.
- **ACCUMULATING — reflects churn, not live series** (a metric with 20,000 `k8s.pod.uid` values may have 50 live pods; the store never reclaims retired series until TTL): `k8s.pod.uid`, `k8s.pod.name`, `k8s.pod.start_time`, `container.id`, `k8s.replicaset.name`.
- **HIGH-but-bounded** — a `valueCount` above ~**100** that is **not** filtered/grouped by any dashboard or alert (confirm usage before flagging; a routing/breakdown label with real consumers is legitimate).
- **DEPLOYMENT_DEPENDENT** — fine at 10, dangerous at 500: `net.peer.name`, `client.address`, `k8s.node.name`, `service.version`, `service.instance.id` (treat as ACCUMULATING if its values are UUID/timestamp-shaped).
- **Unknown attribute decision tree:** user-supplied / request-scoped → UNBOUNDED; changes on pod restart → ACCUMULATING; UUID/hash/hyphen-segmented value → IDENTIFIER; fixed small vocabulary → BOUNDED (fine); count ≈ number of instances → DEPLOYMENT_DEPENDENT.

- **Healthy (SIG-070):** top metrics carry only bounded, consumed labels; no unbounded/identifier label present.
- **Finding (SIG-070, high):** name the metric + the offending label + its observed `valueCount` + a sample-value pattern, and its profile. Fix inline names the OpenTelemetry Collector `metricstransform` processor `aggregate_labels` to **merge** the series (samples are the billable unit) — explicitly **not** a `transform`/`delete_key`, which leaves the same sample count and creates colliding series. Cross-reference the cost view in [audit-cost signoz-cost.md](../../audit-cost/references/signoz-cost.md).
- **Do-not-drop guard (mandatory — a high-cardinality metric is not automatically droppable):** never label these families "safe to trim" even when they have no user dashboard/alert; present them as Infra/APM-page dependencies. **Hosts view:** `system.cpu.*` (incl. `load_average.*`), `system.memory.*`, `system.disk.*`, `system.network.*`, `system.filesystem.*`, `system.paging.*`, `system.processes.*`, `host.cpu.usage`. **Kubernetes view:** `k8s.*` + `container.*` families and their `.uid`/`.name` entity attributes + `k8s.pod.start_time`. **APM/Services:** `signoz_calls_total`, `signoz_latency_*`, `signoz_db_latency_*`, `signoz_external_call_latency_*`. Never suggest trace sampling as a cardinality/cost lever (it makes APM undercount all traffic). For these, the finding is "reduce the *label* cardinality via aggregation," never "drop the metric."
- **ClickHouse read-path optimization (applies to SIG-010/030/070 and the cost reads):** on distributed tables, resolve a resource-attribute filter through a CTE + `resource_fingerprint GLOBAL IN (…)` — a plain `IN`/`JOIN` against a distributed subquery fails under `distributed_product_mode='deny'`. For log body search the two skip indexes are on `lower(body)`, so a predicate must reference `lower(body)` literally (`hasToken(lower(body),'x')` engages the token index; `lower(body) LIKE '%sub%'` with ≥4 chars engages the ngram index; `positionCaseInsensitive` engages neither). Always pair the nanosecond `timestamp` filter with a `ts_bucket_start BETWEEN start-1800 AND end` filter (the `-1800` compensates for 30-minute bucket rounding). When building via the SigNoz query API, a formula's `builder_query` inputs apply their own top-N `limit` **before** the formula evaluates, so a high-ratio group can be dropped — set formula-input limits high (e.g. 10000) and keep only the formula result at 100.
- **Forbidden:** SELECT/read only — never `ALTER`/`OPTIMIZE`/`SYSTEM`, and never apply a `metricstransform`/pipeline change (that is a `setup-*` action behind confirmation; the finding only *names* the fix); see section 9.
...
- **Finding (SIG-040):** a rule that references no channel, references a channel absent from `/api/v1/channels`, or points at an empty/loopback/placeholder destination is the core failure — the rule evaluates and pages nobody. A rule that is disabled or has no threshold/evaluation window evaluates never. **Distinguish a forgotten disable from a deliberate suppression:** a rule left `disabled: true` with **no covering planned-maintenance window** (GET the planned-maintenance list; SigNoz continues evaluating during a maintenance window but suppresses notifications) is a suspected blind spot — someone muted it for a deploy and never re-enabled it — and should be flagged, whereas a rule suppressed by an active or scheduled maintenance window is intended. Prefer a scoped, time-bounded planned-maintenance window over disabling a rule outright. Record the rule name and the channel's host class (e.g. "channel target is a loopback address"), never the full URL. Zero rules entirely routes to SIG-007, not a confident SIG-040 fail. Reading the config proves the path is **configured**, not `validated-live` — the test-fire that would prove delivery is a mutation this audit never performs. Fix inline: SigNoz Alerts → the rule → Notification Channels, and Settings → Alert Channels to create/repair a channel.
...
- **Match-type hysteresis (SigNoz-specific).** SigNoz's `condition.matchType` defaults to **`at least once`** — the rule fires if the threshold is breached at *any single point* in the evaluation window, so one transient spike pages. For a volatile or paging metric prefer **`on average`** or **`all the times`** (the breach must hold across the window), which is the SigNoz equivalent of a Prometheus `for:` dwell. Flag a paging/SLI-bound rule left on the `at least once` default over a volatile signal as flap-prone (medium), paired with the recovery-threshold axis. Read from the same `/api/v1/rules` config (`condition.matchType`), no extra call.

- **Healthy (SIG-042):** SLI-bound rules use burn-rate/multi-window design, carry a recovery threshold, use a windowed match-type (not the `at least once` default) on volatile signals, and their queries return live data.
...
```bash
sig_get /api/v2/dashboards | jq '.data.dashboards'   # v0.138: v2 wraps them in .data.dashboards (v1 is 501 dashboard_deprecated)
```
