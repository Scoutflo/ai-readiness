...
## Phase 7: SigNoz alerting (SIG-040, SIG-042) and dashboards (SIG-041)
...
- **SIG-042 (alert quality — SLO-aware):** beyond "reaches a human," score whether an SLI-bound rule is *worth* the page. When the business-context SSOT defines an SLI for the service (read via the Metadata Load block; degrade to structural-only, and say so, when none is defined), check three axes: a **burn-rate/multi-window** design rather than a flap-prone static threshold on a ratio; a **recovery threshold** (hysteresis) so it doesn't chatter at the boundary; and a **live-signal probe** — one read-only `POST /api/v3/query_range` confirming the rule's own query returns data (the "saves clean, never fires" trap). Full logic in [references/signoz-checks.md §7 (SIG-042)](references/signoz-checks.md#sig-042--alert-quality-slo-aware). Findings *name* the fix (convert to burn-rate / add a recovery threshold / repair the query); SIG-042 never edits a rule.
