...
| --- | --- | --- | --- | --- |
| SigNoz API | `signoz.url` (`http(s)://your-signoz-host:8080`) | `signoz.api_key_env` (the variable named there — e.g. `SIGNOZ_API_KEY`) | a **VIEWER-role** Personal Access Token created in SigNoz Settings → API Keys; used as the header `SIGNOZ-API-KEY: <token>` on every authed `GET` and on `POST /api/v3/query_range` | read-only |
...
if grep -q '^signoz:' "$CFG"; then
  [ -n "${SIGNOZ_API_KEY:-}" ] || { echo "signoz block configured but the API PAT variable (signoz.api_key_env) is not set; the authed SigNoz endpoints need a VIEWER PAT — run /scoutflo:connect"; exit 1; }
else
...
| Admin-scoped or over-privileged audit PAT (SIG-050) | SigNoz UI: Settings → API Keys — issue a VIEWER-role PAT for the audit and revoke the over-scoped one |
...
| A `401` read as a broken endpoint | `401 {"error":{"type":"unauthenticated"}}` on `/api/v1/rules` means the PAT is missing/wrong/not VIEWER — a `blocked` category with the reason, not a confident fail; the open `/api/v1/version` + `/api/v1/health` still prove reachability |
