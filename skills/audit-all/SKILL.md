...
## Fast path: triage mode (for a first look / a short session)

A full deep run over a large estate takes tens of minutes — right for a scheduled run, wrong for a 30-minute session. When the operator asks for a **fast/triage pass** (or sets `SCOUTFLO_TRIAGE=1`), run each configured provider's **high-signal, cheap-to-read subset** per [report-standard/triage-mode.md](../../report-standard/triage-mode.md) (smallest scope, no per-resource fan-out, prefer bulk reads, never background a sweep), then render the **Executive summary** (§2 below) with the worst findings across the estate and offer the deep audit for detail. Triage marks its `findings.json` `scope: "triage"` so a subset is never reported as a full all-clear. Everything else in this skill is the deep run.
...
- Feeds cost-analysis deduplication (Phase 3.7) and `topology-guided-setup` fix sequencing
...
## Phase 3.6: Alert-fatigue roll-up (v0.1.159+)

After correlation completes, run the alert-fatigue roll-up. It aggregates every audit's alerting-noise findings into one estate view, finds services paged by **more than one tool** for a single incident (cross-source storm), and — only when a `fatigue.json` signal block supplies incident counts — computes the alert-to-incident ratio. It reads only this run's `findings.json` files, makes **no provider calls**, never mutates a finding or its severity, and is **not scored**: it cites source finding-IDs, and each noise finding stays scored once in its home audit.
...
AF_LIB="${CLAUDE_PLUGIN_ROOT}/skills/alert-fatigue/lib/alert-fatigue.sh"
if [ -f "$AF_LIB" ]; then
  . "$AF_LIB"
...
## Phase 3.7: Cost roll-up (v0.1.67+)
...
## Phase 3.8: Redaction pass (v0.1.71+)
...
2. **Executive summary (top-of-report one-pager)**: paste the output of `sh "${CLAUDE_PLUGIN_ROOT}/report-standard/render-report-viz.sh" exec-summary "<audits-dir>" "<run-date>"` — a posture grade (AT RISK / NEEDS WORK / FAIR / HEALTHY) + severity counts, the **top 5–7 worst findings across the whole estate** each as *what · where (blast radius) · $ · the fix*, the measured **alerting-reachability** headline (AF-004), and the single top real cost lever. Ranked **severity-first, then recoverable points, then $ as an in-band tiebreaker — never a blended cross-domain score, and $ never promotes a lesser finding above a critical one** (CVSS / AWS Security Hub both keep severity separate from $/criticality). So a leader sees the worst things and their fix in 30 seconds. Renders only artifacts already on disk; degrades to a one-line note when no findings exist yet. Part of the fast [triage-mode.md](../../report-standard/triage-mode.md) doctrine.
3. **At a glance (all stacks)**: paste the output of `sh "${CLAUDE_PLUGIN_ROOT}/report-standard/render-report-viz.sh" rollup "<audits-dir>" "<run-date>"` — a gate-count meter (stacks passing the 85 gate / total) and a worst-first per-stack score-bar table, so a leader sees estate health and which stack to send the team to first at a glance. Never a combined average (that hides a failing stack behind a healthy one); this only visualizes the per-target scores.
4. **Scores**: one row per completed target: readiness (`n/100` or `unassessed`), v2 assessment coverage, severity counts, estate size and sizing path (from the Phase 3 roll-up; `estate not recorded` when the audit did not emit one), delta versus that target's previous compatible run (`+9`, `-3`, `first run`, or `not comparable`), the compatible score trend from its `history.jsonl` (last five runs, oldest first), and the relative path to its full `report.md`. Score movement is comparable only when `scoring_model` and `check_set` match. One score line per target, never a combined average: an average hides a failing stack behind a healthy one.
5. **Blocked audits**: one row per blocked audit with reason and fix pointer. Omit the section only when nothing was blocked, and then state "No audits blocked."
6. **Regressions**: the Phase 3 regressions list, one row per regressed finding with its target, ID, severity, and title, ordered highest severity first. State "No regressions this run." when the list is empty. This section always precedes Top findings; regressions are the highest-signal state in the lifecycle model and are named before anything else.
7. **Topology Readiness (combined)**: one row per completed target, its "`<r> of <n> critical services are ready for automatic Scoutflo correlation`" headline from the Phase 3 topology-readiness roll-up, and a link to that target's own Scoutflo Topology Readiness section for the per-service detail. A target whose roll-up line read `readiness not recorded` gets that exact phrase in its row, not a blank or a guess.
...
VIZ="${CLAUDE_PLUGIN_ROOT}/report-standard/render-report-viz.sh"
sh "$VIZ" exec-summary     "$AUDITS_DIR" "$RUN_DATE"     # -> paste as the "Executive summary" section (top of report)
...
3. **The biggest levers across the estate:** the two or three highest-`points_recoverable` finding titles, and regressions first if any.
4. **Where the combined report is — the resolved ABSOLUTE path:** `<abs>/all/<YYYY-MM-DD>/report.md`, plus a note that each target's own `report.md` sits under `<abs>/<target>/<YYYY-MM-DD>/`.
5. **How to open it** (OS-specific `open`/`xdg-open`/`Invoke-Item`, as in the report standard), noting it is plain Markdown.
6. **How to share it:** the full reports name hosts/namespaces/routes — share inside the team; the Slack brief (already sent if `slack.webhook_env` is set, else offer it) is the leak-safe summary.
7. **Next step:** re-run after fixes for the delta, or open the target report with the most recoverable points first.
