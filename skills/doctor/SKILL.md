...
| Field | Meaning |
| --- | --- |
| `integration` | Template-order row: toolkit, grafana, sentry, pagerduty, datadog, elk, jsm, zenduty, groundcover, prometheus, alertmanager, loki, tempo, mimir, victoriametrics, vmalert, digitalocean, gcp, aws, kubernetes, slack |
| `check` | What was checked: `env` (variable presence), a live endpoint (e.g. `health`, `identity`, `org`, `abilities`, `analytics`, `validate`, `monitors-read`, `cloudwatch-read`, `cost-permissions`, `alerting-health`, `alerts-read`, `teams-read`, `spaces`, `query`, `status`, `config`, `ready`, `rbac`, `account`, `vmalert-health`, `monitoring-api`, `webhook-post`), `binary-*`, or `configured` for unconfigured rows |
