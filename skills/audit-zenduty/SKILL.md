...
Commands in section 8. Zenduty exposes **server-side analytics**, so — unlike some paging providers in this toolkit — this category rests on the vendor's own measured statistics, not a client-side reconstruction. The incident-filter capture retains per-incident `service`/`service_ids` and `sla_object` (see section 8's required capture change) so these checks join to cause, not just count. Unacknowledged incidents older than the aging threshold from the incident filter, with ack state read from `status` (`ZD-030`, high — every one is a page nobody took; join each aging incident to its service's `escalation_policy` and that EP's `oncall_user_count` from oncall.json to state *why* it aged — empty rotation (ZD-003) vs buried in noise (ZD-010) — making ZD-030 the live-proof leg of the dead-path cascade); MTTA against the service's OWN `sla_object.acknowledge_time` (a per-incident field on the incident filter, falling back to `MTTA_TARGET_MIN` only when absent — analytics carries no SLA), then attributing the miss by correlating with ZD-010/ZD-014 for that service (`ZD-031`); and MTTR against target from `mttr_seconds` alongside the acked/resolved share (`ZD-032`). All figures carry the analytics window in evidence.

- ❌ `Actionability: roughly 3% of pages appear actionable (industry benchmark).`
- ✅ `ZD-031 fail for checkout: mtta_seconds 2140 (36 min) over 30 days on 88 acked incidents against checkout's own sla_object.acknowledge_time of 900s (incident filter) — and checkout has collation:0 (ZD-010), so the miss is noise, not staffing; affected: checkout.`
...
| ZD-031 compared against a hardcoded target | Prefer the service's own `sla_object.acknowledge_time` (a per-incident field on the incident filter); `MTTA_TARGET_MIN` is only the fallback when `sla_object` is absent — analytics carries no SLA field |
