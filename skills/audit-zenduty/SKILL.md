...
- **Two integer enums are UNVERIFIED and gate two verify-pending checks.** The escalation-rule `target_type` int → `{user, schedule}` mapping (ZD-006) and the alert-rule `action_type` int → action-name mapping (ZD-017) are not confirmed against a live tenant; both fields are real, but read the action/target by name/`key` and confirm the enum before flagging. ZD-006 and ZD-017 are drafted against the documented API and adversarially reviewed but NOT run against a live tenant — treat their status as unproven until a first live run with a read-only token, and never emit a fabricated live observation for them.
...
| Escalation level targets a lone individual (ZD-006, verify-pending) | Zenduty > Teams > (team) > Escalation Policies — repoint the level at an On-Call Schedule instead of a named user |
...
| ZD-006/ZD-017 flagged from an assumed integer enum | The `target_type` (ZD-006) and `action_type` (ZD-017) integer enums are UNVERIFIED against a live tenant; read the target/action by name or `key`, confirm the enum on a first live run, and treat both checks as verify-pending until then — never emit a fabricated live observation |
