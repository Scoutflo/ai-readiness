...
This skill audits the Zenduty paging layer, not the monitoring tools that *send* events into it. The `for` durations, recovery thresholds, and grouping of those upstream tools belong to their own audits (`audit-lgtm`, `audit-grafana`, `audit-sentry`, `audit-datadog`, `audit-alertmanager`); this audit picks up where an event has already arrived at Zenduty.
...
- **`GET /api/incidents/` is WAF-blocked (`HTTP 209`, body `Blocked`), not create-only.** Live-confirmed 2026-09-01. The read-by-POST `POST /api/incidents/filter/` (a filter body, changes nothing) and the paginated `GET /api/v2/incidents/` (also live-confirmed: `{count,next,previous,results}`, fixed page size of 10 regardless of a `page_size` query param) are both real read paths for the incident stream; `GET /api/incidents/` is neither. Ack state is derived from the incident `status` integer (`1` triggered/unacked, `2` acknowledged, `3` resolved; `-1` open), not a boolean field.
- **Teams live at `/api/account/teams/`; `/api/teams/` is a plain `404`.** Live-confirmed. There is no unversioned teams collection.
- **`GET /api/account/teams/{team}/priorities/` can `404` per tenant** when the team-priorities feature is absent; treat that `404` as `not-in-scope`, never a broken call.
...
- **Maintenance windows are `{repeat_interval, repeat_until (nullable), services[]}`.** A window with `repeat_interval` non-zero and `repeat_until: null` is an open-ended recurring suppression. The capture retains the full `services[]` (not just a count) so ZD-016 can name which covered services are critical.
- **The escalation-rule `target_type` int → `{user, schedule}` mapping is live-confirmed (ZD-006).** `target_type == 2` with a `target_meta.email` present is a named USER (a schedule target carries no personal email in `target_meta`); this is no longer verify-pending. The capture derives a **redacted boolean** `is_user` from `target_meta.email` presence at capture time — the real email is never written to disk, evidence, or the report.
- **One integer enum remains UNVERIFIED and gates one verify-pending check.** The alert-rule `action_type` int → action-name mapping (ZD-017) is not confirmed against a live tenant; the field is real, but read the action by its `key`/name and confirm the enum before flagging. ZD-017 is drafted against the documented API and adversarially reviewed but NOT run against a live tenant — treat its status as unproven until a first live run confirms the `action_type` enum, and never emit a fabricated live observation for it.
...
## Phase 3: Escalation and on-call (ZD-001 to ZD-008)

Commands in section 5. Judge whether a page reaches a reachable human — and for every scored finding compute a real blast radius, not a shape assertion: join `services.json`'s `.escalation_policy` back to the offending policy to count *which* services (and which critical ones from topology) actually page through it, and cite ZD-030's aging incidents as the live proof a backstop already failed. Every audited team has an escalation policy and no production team's policy is a single rule with one target and `repeat_policy: 0` (`ZD-001`, critical when missing, high on the SPOF shape — state the services routing through it); escalation actually advances (`ZD-002` reads `move_to_next`: a multi-level policy with `move_to_next:false` never progresses past level 1, so its level-2 targets are decorative — name the services that then escalate to nobody); every escalation policy's on-call resolves to a staffed rotation now — an empty `oncalls` user array pages nobody (`ZD-003`, high); schedules referenced are non-empty and cover the window (`ZD-004`); no integration on a live ingestion path sits `is_enabled: false` so nothing can create alerts and the service goes fully dark (`ZD-005`, critical — count enabled integrations per service and intersect the zero-enabled ones with critical services); no escalation level targets a lone named individual instead of a rotation/schedule (`ZD-006`, high — `target_type == 2` with `target_meta.email` present is a live-confirmed USER target); no escalation policy is orphaned dead config with `connections == 0` and no service joining it (`ZD-007`, medium — the opposite failure from ZD-001, a policy nobody uses rather than a bad-shaped policy somebody depends on); and, account-wide across every audited team's escalation targets, more than one distinct human backs escalation — the redacted `is_user` boolean lets this run without ever reading a real identity (`ZD-008`, high — a low distinct-human count is the account-level fact that per-EP ZD-006/ZD-003 checks can each pass locally while still sharing the same single point of failure).

- ❌ `Escalation pass: every team names a policy.`
- ✅ `Escalation partial: the payments EP is one rule, one user, repeat_policy 0 (ZD-001); 6 services route through it (services.json join), 2 critical (checkout, payments); its primary escalation has an empty on-call now (ZD-003) and 9 checkout incidents already sit triggered >4h (ZD-030); affected: checkout, payments.`

## Phase 4: Alert noise (ZD-010 to ZD-017)

Commands in section 6. This is the alert-hygiene category; every scored finding here carries a live count, not an "X is off" scanner line. Per-service `collation` on where a service is chatty — and PROVE the chattiness from `service_analytics` `total_incidents` vs `total_acknowledged` for that service (e.g. "checkout: collation:0, 412 incidents / 27 acked in 30d"), remembering the API reads time-based-vs-off and content-based correlation is not API-readable (`ZD-010`); suppress alert rules present for known-noisy sources and reviewed for over-broad matchers that silently drop real alerts (`ZD-011`, high — a suppress rule with an always-true condition is an accidental drop-all); the "Seconds Since Last Similar Incident" condition used to guard flapping where a source re-fires after resolution (`ZD-012`); delay-notification rules deliberate rather than blanket off-hours muting (`ZD-013`); a stable, non-blank `entity_id` so the default dedup collapses repeats — a rule that blanks or randomizes `entity_id` disables dedup (`ZD-014`); auto-acknowledge/auto-resolve working because sources emit resolve `alert_type`s rather than only ever firing (`ZD-015`); no maintenance window that is an open-ended recurring suppression — `repeat_interval` set with `repeat_until: null` (`ZD-016`, high — expand the retained `services[]` to name which covered services are critical and how many days the blackout has run); and no alert rule that downgrades a critical-service incident's urgency to non-paging, keeping the incident but never waking anyone (`ZD-017`, high, **verify-pending** — the "Change Incident Urgency" action is real but its `action_type` enum is unproven live).
...
## Phase 5: Coverage and hygiene (ZD-020 to ZD-025)

Commands in section 7. Global alert routing has no overlapping or duplicate routes that fan one alert to multiple services, and a default route exists (`ZD-020` — name the specific duplicated service pair the overlap double-pages, and name the unmatched-alert class a missing catch-all silently drops; presence-only "no default" is a scanner line); critical services from topology each covered by a team, a service, and an escalation path (`ZD-021`, high — the flagship anchor); the audited teams named and any team not audited named as uncovered rather than silently dropped (`ZD-022`); and no integration still on the deprecated "API-Integration" ingestion type that stopped working 2025-05-15 (`ZD-023`, migration debt — more dangerous than ZD-005 because `is_enabled` stays true so the service *looks* monitored while every event from that source is dropped at ingestion; name the service and its criticality). Teams must be visible to this key at all — zero teams visible trips the empty/hidden-teams guardrail (`ZD-024`, high; the Case-B guardrail), which `blocks` the team-scoped categories with the visibility-gap reason rather than scoring a confident empty. Account dormancy is named as its own informational fact — the most recent incident's `creation_date` from `GET /api/v2/incidents/` older than `DORMANCY_DAYS` reads "this scorecard describes a dormant tenant, not a recently-tested one" (`ZD-025`, info — never lowers another category's score).

## Phase 6: Actionability (ZD-030 to ZD-034)

Commands in section 8. Zenduty exposes **server-side analytics**, so — unlike some paging providers in this toolkit — this category rests on the vendor's own measured statistics, not a client-side reconstruction. The incident-filter capture retains per-incident `service`/`service_ids` and `sla_object` (see section 8's required capture change) so these checks join to cause when the incident carries a service — integration-created incidents may return `service`/`service_ids` `null` (live-confirmed), in which case the aging count still stands and attribution degrades to the account/EP level. Unacknowledged incidents older than the aging threshold from the incident filter, with ack state read from `status` (`ZD-030`, high — every one is a page nobody took; join each aging incident to its service's `escalation_policy` and that EP's `oncall_user_count` from oncall.json to state *why* it aged — empty rotation (ZD-003) vs buried in noise (ZD-010) — making ZD-030 the live-proof leg of the dead-path cascade); MTTA against the service's OWN `sla_object.acknowledge_time` — **in minutes, so ×60 before comparing to `mtta_seconds`** — (a per-incident field on the incident filter, falling back to `MTTA_TARGET_MIN`, also minutes, only when absent — analytics carries no SLA), then attributing the miss by correlating with ZD-010/ZD-014 for that service (`ZD-031`); and MTTR against target from `mttr_seconds` alongside the acked/resolved share (`ZD-032`). All figures carry the analytics window in evidence.

When the analytics POST is unavailable (a read-only key that can't reach it, a plan gate, a transient failure), ZD-031/ZD-032 lose their category — `ZD-033` proves the same shape is computable straight from the plain, live-confirmed `GET /api/v2/incidents/` list: per-service acked-share, resolved-never-acked-share (an "ignored alert" — `resolved_date` set, `acknowledged_date` null), and still-open-share (`status` still triggered, never acked), paged via `next` (fixed page size of 10). When analytics is reachable, ZD-033 is corroborating evidence cited alongside ZD-030/031/032, never a second filing of the same cause; when it is not, ZD-033 is this run's Actionability evidence and the report says so. And no service leaves its own incidents to pile up forever: a service with `auto_resolve_timeout == 0` **and** `acknowledgement_timeout_enabled == false` has no timeout backstop on either side of the ack, which is the concrete mechanism behind an ever-growing ZD-030 aging count, not just a correlated symptom (`ZD-034`, medium).

- ❌ `Actionability: roughly 3% of pages appear actionable (industry benchmark).`
- ✅ `ZD-031 fail for checkout: mtta_seconds 2140 (36 min) over 30 days on 88 acked incidents against checkout's own sla_object.acknowledge_time of 15 min (900s target; incident filter) — and checkout has collation:0 (ZD-010), so the miss is noise, not staffing; affected: checkout.`

### Flagship correlation: the dead-paging-path cascade

Assemble the highest-value finding no free Zenduty view produces — for a named critical service, the exact link where its page dies tonight, proven by the live incident stream. Traverse the paging graph and chain the legs into one named finding (each leg naming the others in its evidence), ranked by `points_recoverable` and led in the executive summary: critical service (ZD-021) → its only integration is `is_enabled:false` or on the dead API-Integration type (ZD-005/ZD-023) → **or** it routes to a SPOF / `move_to_next:false` / lone-user escalation policy (ZD-001/ZD-002/ZD-006) → whose on-call rotation is empty now (ZD-003) → **or** a stale open-ended maintenance window silences it (ZD-016) → **or** an alert rule downgrades its urgency to non-paging (ZD-017) → and the live proof is ZD-030's aging unacked incidents joined to that exact escalation policy's empty on-call. Every leg is a real join from the raw capture (`services.json.escalation_policy`, `oncall.json.oncall_user_count`, `maintenance.json.services[]`, the incident filter, `service_analytics`); the full write-up is [references/zenduty-checks.md](references/zenduty-checks.md) section 14. ZD-006 is now live-confirmed; ZD-017 remains a verify-pending leg — mark it so and never fabricate a live observation for it. Where they apply to the same service, name ZD-007/ZD-008 (an orphaned sibling policy or an account-wide bus factor of 1 explaining why the escalation leg has no real fallback) and ZD-034 (why the aging count never self-corrects) alongside the cascade.
...
Score per [severity-and-scoring.md](../../report-standard/severity-and-scoring.md): each check yields `pass` (1.0), `partial` (0.5), or `fail` (0). `blocked` is unassessed and leaves the readiness denominator; `not-in-scope` leaves both readiness and assessment-coverage denominators. Category score is the assessed-credit ratio times 100 rounded down; overall is the weight-normalized sum over categories with at least one assessed check. A team or API surface that returns 401/403/404, or a category the rate limiter blocked mid-run, is blocked with the exact state, never failed or empty. Show assessment coverage separately. A fully blocked run is `unassessed` with `overall: null`, never 0/100. Score conservatively: when unsure between two results, pick the lower and say why. Assign each category a maturity value (`reactive`, `proactive`, `systematic`).

| Category | Weight | ID range |
| --- | ---: | --- |
| Escalation and on-call | 30 | ZD-001 to ZD-008 |
| Alert noise | 25 | ZD-010 to ZD-017 |
| Coverage and hygiene | 20 | ZD-020 to ZD-025 |
| Actionability | 25 | ZD-030 to ZD-034 |
...
End-to-end gate: claim end-to-end coverage only when the overall score is at or above 85, assessment coverage is 100%, every critical service passes every applicable coverage row, and no category or team was excluded. Below the gate, write "good base coverage", never "end to end". A run the rate limiter forced to skip some teams cannot claim end-to-end; say which teams the claim rests on.
...
4. Emit one `checks[]` row for every stable `ZD-*` catalog check, including passes, partials, failures, blockers, and not-in-scope checks. Derive category counts, readiness, assessment coverage, and `score.check_set` from that complete ledger; never write them independently.
5. Every finding declares `scoring_scope: "readiness"` and `report_lanes`: `general-audit`, `ai-sre-readiness`, or both. Use the AI SRE lane only when the evidence shows impact to telemetry quality, service identity/naming, topology/ownership context, incident routing evidence, RCA trust, or action safety — a coverage, naming, or routing-evidence gap is typically both, a pure reliability/cost/security-posture gap is `general-audit` only. This classification never changes severity or score.
...
# ZD-* catalog check, lifecycle + scoring_scope + report_lanes set per finding), inventory.json,
# and report.md per the report standard. The findings.json ".target" is the per-target slug
# (equal to $ZD_SEG: "zenduty" for a single block, "zenduty/<label>" for a labeled list target),
# so audit-all/correlation/render disambiguate multiple Zenduty accounts. Verify:
jq -e --arg seg "$ZD_SEG" '.schema == "scoutflo-findings/v2" and .target == $seg
...
LINE="$(jq -c --arg d "$RUN_DATE" --argjson resolved "$RESOLVED" \
  '{run_date:$d, skill:"audit-zenduty", overall:.score.overall, state:.score.state,
...
| --- | --- |
| Missing or SPOF escalation (ZD-001, ZD-002) | Zenduty > Teams > (team) > Escalation Policies — add a second target, a level, a repeat, or set `move_to_next` so a no-ack advances |
| Escalation level targets a lone individual (ZD-006) | Zenduty > Teams > (team) > Escalation Policies — repoint the level at an On-Call Schedule instead of a named user |
| Orphaned escalation policy, no service joins it (ZD-007) | Zenduty > Teams > (team) > Escalation Policies — delete the unused policy or wire it to the service it was meant for |
| Account-wide bus factor of 1 across escalation targets (ZD-008) | Zenduty > Teams > (team) > Escalation Policies / On-Call Schedules — spread escalation targets across more of the team or add staffed schedules so no one human backs most of the account |
...
| Alert rule downgrades critical-service urgency to non-paging (ZD-017, verify-pending) | Integration > Alert Rules — remove or scope the Change-Incident-Urgency action so critical-service incidents keep high/paging urgency |
...
| Account dormancy, no recent incident (ZD-025, informational) | None required — confirm upstream monitoring tools are still sending events if the dormancy is unexpected (their own audits name that) |
| Unacked aging, slow MTTA/MTTR per analytics or the GET-only fallback (ZD-030 to ZD-033) | On-call process review; fix the noisy sending tool (its own audit names it) or this account's collation/routing |
| No ack/auto-resolve timeout backstop on a service (ZD-034) | Service > Basic Settings — set a deliberate acknowledgement timeout or an auto-resolve timeout appropriate to the service |
...
| Incident list fetched from `GET /api/incidents/` | That path is WAF-blocked (`HTTP 209`, body `Blocked`), not create-only; use `POST /api/incidents/filter/` (a read-by-filter) or the paginated `GET /api/v2/incidents/` |
| `page_size` assumed to control the `/api/v2/incidents/` page size | Live-confirmed: the page size is fixed at 10 and a `page_size` query param is ignored; page via `next` and budget the page count accordingly |
| `/api/teams/` used to list teams | That path 404s; teams live at `/api/account/teams/` |
| A 404 on `.../priorities/` treated as a broken call | The team-priorities feature 404s per tenant when absent; record it `not-in-scope`, never a failed check |
...
| ZD-017 flagged from an assumed integer enum | The `action_type` (ZD-017) integer enum is UNVERIFIED against a live tenant; read the action by name or `key`, confirm the enum on a first live run, and treat the check as verify-pending until then — never emit a fabricated live observation. ZD-006's `target_type` enum is now live-confirmed and no longer carries this caveat |
| Blast radius stated as an adjective ("risky", "chatty") | Every scored finding computes a count/named set from a real join: services routing through an EP (services.json `.escalation_policy`), incidents vs acked (service_analytics), covered services (maintenance `services[]`), aging incidents joined to an empty on-call (incident filter + oncall.json) — never "X is off" alone |
| ZD-031 compared against a hardcoded target, or units mixed | Prefer the service's own `sla_object.acknowledge_time` (a per-incident field on the incident filter, **in minutes — ×60 before comparing to the seconds-valued `mtta_seconds`**); `MTTA_TARGET_MIN` (also minutes) is only the fallback when `sla_object` is absent — analytics carries no SLA field |
| A real email written into `escalations.json` or evidence for ZD-006/ZD-008 | Capture only the redacted `is_user` boolean (`target_meta.email` presence, computed at capture time), never the email or name itself; ZD-008 compares opaque `target_id` values, never an identity |
| ZD-033 filed as a second, duplicate finding alongside ZD-030/031/032 on the same service | Cross-reference, don't double-file: when analytics is reachable, ZD-033 corroborates ZD-030/031/032 under their own IDs; only when analytics is unavailable does ZD-033 carry the category on its own |
| Account dormancy (ZD-025) scored as a category failure | It is informational only — it never lowers Escalation/Alert-noise/Coverage/Actionability scores, it names the fact that the account's recent history is thin |
