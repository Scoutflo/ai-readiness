...
| `/scoutflo:audit-lgtm` | audit | Scored audit of Prometheus, LGTM, and VictoriaMetrics stacks — the LGTM/VictoriaMetrics stores and per-service coverage (for the deep Prometheus server + rule-engine plane use `audit-prometheus`; pair with `audit-alert-routing` for the Prometheus→Alertmanager paging path) |
