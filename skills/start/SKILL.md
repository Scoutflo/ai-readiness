...
3. `/scoutflo:map-topology` (recommended) writes `./scoutflo-audits/topology.md`, your service map — from Kubernetes/Istio when you have a cluster, otherwise from cloud inventory + APM or a guided capture; on AWS/DigitalOcean/Azure/GCP it can also map the databases/caches/queues behind your services and the connections to them, reviewed with you before anything lands. With it, findings name your actual services instead of generic ones. On large estates it works in bounded batches and resumes if interrupted.
4. Run your first audit: `/scoutflo:audit-prometheus` (the deep **Prometheus** server + rule-engine plane), `/scoutflo:audit-lgtm` (LGTM/VictoriaMetrics stores; pair with `/scoutflo:audit-alertmanager` for the Prometheus→Alertmanager paging path), `/scoutflo:audit-grafana`, `/scoutflo:audit-sentry`, `/scoutflo:audit-pagerduty`, `/scoutflo:audit-datadog`, `/scoutflo:audit-elk`, `/scoutflo:audit-jsm`, `/scoutflo:audit-zenduty`, `/scoutflo:audit-groundcover`, `/scoutflo:audit-clickstack`, `/scoutflo:audit-signoz`, `/scoutflo:audit-newrelic`, `/scoutflo:audit-kubernetes`, `/scoutflo:audit-alertmanager`, `/scoutflo:audit-digitalocean`, `/scoutflo:audit-gcp`, `/scoutflo:audit-azure`, `/scoutflo:audit-aws`, or `/scoutflo:audit-cost` (cross-provider cost). Or run everything configured at once with `/scoutflo:audit-all`. Every audit sizes your estate first with cheap list calls and says which path it picked: small estates get a single pass, large ones run in bounded, resumable batches.
...
| `/scoutflo:migration-plan` | harness | Read-only migration plan between providers (currently Datadog → SigNoz): evidence-cited migrate/fix/drop/covered/no-equivalent inventory + cutover playbook; plans only, changes nothing |
...
| `/scoutflo:audit-lgtm` | audit | Scored audit of Prometheus, LGTM, and the VictoriaMetrics family (VictoriaMetrics + VictoriaLogs via the `loki:` block + VictoriaTraces via the `tempo:` block, auto-detected) — the stores and per-service coverage (for the deep Prometheus server + rule-engine plane use `audit-prometheus`; pair with `audit-alertmanager` for the Prometheus→Alertmanager paging path) |
...
| `/scoutflo:audit-newrelic` | audit | Scored audit of a New Relic account: alert delivery (condition→policy→workflow→destination), alert noise incl. measured NrAiIncident fire-history, entity coverage + span-derived topology, SLO/dashboard posture, data health (NrIntegrationError, cardinality), plus non-scored ingest/cost |
