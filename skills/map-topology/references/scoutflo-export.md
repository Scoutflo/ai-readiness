...
- `confidence` 0-10. Use 9 for object-backed edges, 8 for user-declared watchpoints, lower when uncertain. 8 is the actionable threshold downstream, but the number alone is not sufficient: the real correlation engine also requires a service/workload/app identity attribute plus either a Kubernetes anchor (`namespace`/`pod`/`container`) or, for Sentry, a `project`/`environment` attribute, before treating the edge as actionable rather than `warning_only`. Populate the identity and anchor attributes together, not just a high confidence number on its own.
...
Contract note: aligned to the import contract as of 2026-07. Enums are strict on import; re-verify against current Scoutflo documentation before running an actual import.
