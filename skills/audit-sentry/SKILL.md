...
4. **Environment-name sprawl (SNTRY-025).** SNTRY-004 confirms one required name exists; it does not confirm the project's environment list is otherwise coherent. From the same `/environments/` list, check for synonym collisions against a canonical map (`prod`+`production`, `pp`+`preprod`+`pre-prod`, `stage`+`staging`): a project carrying two members of the same family means half its traffic tags itself with the name a rule scopes to and the other half does not, so an `environment`-scoped rule (SNTRY-102, SNTRY-013) only ever sees half the picture, silently. Command and live evidence in [references/api-checks.md](references/api-checks.md#depth-checks-per-project-quota-attribution-deterministic-source-map-verdicts-vcscode-mapping-integrity-monitor-posture-coverage-split-environment-sprawl-sntry-018-through-sntry-025).
...
6. **Disabled or muted rules (SNTRY-016, live-verified; metric-alert half verify-pending).** A rule present in `rules.json` but with `status != active` fires nothing, yet before this wave still satisfied SNTRY-013's tier-presence test — so a service read as covered while its immediate-tier rule was switched off. List every non-active issue rule and cross-check its conditions against the tiers; a disabled immediate rule on a production project is the finding, quantified with the project's accepted-error volume. The issue-rule half is live-verified (issue rules carry a string `status` — observed `active`). The metric-alert `status` is a numeric enum (confirmed live — observed `0` on active rules), so the string-typed guard never fires there; the disabled-enum value stays verify-pending until a disabled metric alert is observed. See [references/api-checks.md](references/api-checks.md#disabledmuted-rules-and-owner-routing-gaps-sntry-016-sntry-017).
7. **Owner-routing gap (SNTRY-017, live-verified).** A rule action with `targetType: IssueOwners` delegates delivery to code-path owners; with empty ownership `raw`, delivery falls through — to everyone (noise, mirrors SNTRY-001) or, fallthrough off, to nobody (silent). A 403 on `/ownership/` blocks the check, never a clean pass. Live-verified: `/ownership/` returns `200` with `raw` and a boolean `fallthrough`. See [references/api-checks.md](references/api-checks.md#disabledmuted-rules-and-owner-routing-gaps-sntry-016-sntry-017).

## Phase 4: Integrations and source context

Checks SNTRY-009, SNTRY-021, SNTRY-022 (snippets in [references/api-checks.md](references/api-checks.md#integration-and-source-context-checks) for SNTRY-009; [references/api-checks.md](references/api-checks.md#depth-checks-per-project-quota-attribution-deterministic-source-map-verdicts-vcscode-mapping-integrity-monitor-posture-coverage-split-environment-sprawl-sntry-018-through-sntry-025) for SNTRY-021/022). Inventory integrations by provider and status. Then judge the VCS chain as a chain, not as parts: a repo integration without code mappings resolves nothing; a code mapping whose `defaultBranch` differs from the branch you actually deploy resolves the wrong file; releases with `commitCount: 0` mean suspect commits have no data even though the integration looks connected. Each broken link is the finding; "GitHub is connected" alone earns no credit.

- **Repo integrity (SNTRY-021).** SNTRY-009's step 1 only asks whether *an* active VCS integration exists; it does not inspect every row in `repos.json`. A repo entry whose `provider.id` is not a real `integrations:*` provider (a generic/unknown provider, no `integrationId`) is a `sentry-cli set-commits` misconfiguration sitting alongside legitimate connected repos — it can coexist with a passing SNTRY-009 step 1. Escalate when a release's commits bind to that repo's numeric id.
- **Code-mapping plausibility (SNTRY-022).** Beyond SNTRY-009's presence/branch/commit checks, inspect the mapping set itself for structural implausibility: two mappings for the same `{project, repo}` pair whose `stackRoot` differs only by a `./` segment (a redundant, unordered duplicate), a `stackRoot` rooted under a build-host path (`/tmp`, `/home`, `/Users`) that never survives past the machine that generated it, or one project routing more mappings into a single repo than your declared ceiling.

## Phase 5: Releases and source maps

Checks SNTRY-006, SNTRY-020. Release health signals, judged live:
...
3. Minified frames (SNTRY-006): sample the latest event of a recent issue in each JavaScript or TypeScript project and inspect in-app frames. Bundled filenames with no source context mean source maps are not resolving, whatever the upload pipeline claims. Command in [references/api-checks.md](references/api-checks.md#releases-and-source-map-checks); requires `event:read`, otherwise record the check `blocked`.
4. **Deterministic symbolication verdict (SNTRY-020).** SNTRY-006 is a frame-shape heuristic; the same sampled event's top-level `errors[]` array carries an explicit, server-stated symbolication failure (`type: "js_no_source"` or similar) when resolution failed — read it as the stronger, non-heuristic verdict, and independently probe `/projects/{org}/{project}/files/artifact-bundles/` as upload-pipeline proof that does not depend on which event happened to be sampled. A null `release` inside an artifact bundle's `associations[]` is the normal shape for a debug-ID bundle and is never read as "upload missing." Bundles present with `errors[].type == "js_no_source"` still on a sampled event is its own, more specific finding — the pipeline runs but resolution fails for another reason (wrong `dist`, wrong URL prefix, coverage gap). Command and live evidence in [references/api-checks.md](references/api-checks.md#depth-checks-per-project-quota-attribution-deterministic-source-map-verdicts-vcscode-mapping-integrity-monitor-posture-coverage-split-environment-sprawl-sntry-018-through-sntry-025).

## Phase 6: Cron and uptime monitors

Checks SNTRY-007, SNTRY-023. From `monitors.json` plus a re-fetch per suspect monitor. Do not stop at "monitor has no check-ins" — name the underlying job and whether a critical service depends on it, so the finding reads as a fire, not a scanner row:
...
- **Zero-monitor posture (SNTRY-023).** The checks above only judge monitors that exist; `monitors.json == []` says nothing on its own — many estates genuinely run no scheduled jobs. But when a backend (non-browser-platform) project carries real accepted-event volume in the same window and the org has zero monitors of any kind, that absence is worth an explicit line: report an `info`-severity finding naming the volume-carrying project, never a silent pass and never folded into SNTRY-007's denominator (SNTRY-007 has nothing to score when the list is empty). Command and live evidence in [references/api-checks.md](references/api-checks.md#depth-checks-per-project-quota-attribution-deterministic-source-map-verdicts-vcscode-mapping-integrity-monitor-posture-coverage-split-environment-sprawl-sntry-018-through-sntry-025).

## Phase 7: Volume, quota, and privacy-sensitive ingestion

Checks SNTRY-008, SNTRY-010, SNTRY-018, SNTRY-019, from `stats-outcomes.json` and `stats-projects.json`:
...
3. **Per-project drop ratio (SNTRY-018).** SNTRY-008 reads the org-wide outcome totals, so a single hot project's drops can sit comfortably inside a healthy org-wide average and never surface. Re-read the same window grouped by numeric project id (`stats-projects.json`, already pulled in Phase 1) and apply the same `DROP_RATIO` floor per project. A project over the floor is the finding even when SNTRY-008 passes; name the project by slug (resolved from `projects.json`), not the numeric id.
4. **Rate-limit reason split (SNTRY-019).** The `stats_v2` `reason` dimension distinguishes *why* an `outcome: rate_limited` drop happened: Spike Protection (a burst-shape control) versus a per-key or cardinality quota breach (SNTRY-003's object). The two need different fixes; conflating them misdirects remediation. Never attribute a specific drop to a specific key — the read is org- or project-wide, never per-key.

Command and live evidence for SNTRY-018/019 in [references/api-checks.md](references/api-checks.md#depth-checks-per-project-quota-attribution-deterministic-source-map-verdicts-vcscode-mapping-integrity-monitor-posture-coverage-split-environment-sprawl-sntry-018-through-sntry-025).

## Phase 7b: Alert hygiene (noise posture)

Phases 3 through 8 prove a page can reach a human and that critical services are covered. This phase asks the opposite: is what reaches on-call tuned, or has Sentry been left on defaults that notify on every trigger, across every environment, on every ingested bot event? Every check re-reads objects the earlier phases already touch — the per-project `rules.json`, the org `metric-alerts.json`, and `stats-outcomes.json` — plus reads Phase 1 does not do: `/projects/{org}/{project}/filters/` per project, a stats `reason` breakdown, top unresolved issues per project (`/projects/{org}/{project}/issues/`), and per-rule fire history (`/projects/{org}/{project}/rules/{ruleId}/stats/` — one GET per rule, fetched only for rules a structural check already flagged). Two fields the rules list already returns for free carry the rest: `lastTriggered` (ISO or null) and `owner` (`team:…`/null). Full commands are in [references/api-checks.md](references/api-checks.md#alert-hygiene-noise-control-checks). Checks SNTRY-101 through 110 join the existing Alert rules and routing category: they grow its denominator and do not re-weight it. SNTRY-106 adds measured fire counts and never-fired detection as the shared data source; SNTRY-107 is the flagship chronic-issue re-page join that answers *why a channel is unusable* rather than *which field is null*.
...
- The SNTRY-106/107 **fire counts measure emission, not annoyance**: a rule that fired 1,681 times sent 1,681 notifications, but with no incident/ack feed the audit cannot claim how many were acted on — it never converts a fire count into an actionability rate. **"Never fired" is bounded by the ~90-day stats window** — state it as "not in the observable window," not as proof a rule is permanently dead. The SNTRY-107 re-page numbers are **legal maxima** (`matching_rules × 1440/frequency`), not observed counts; when `/stats/` data exists, quote both (ceiling and observed) and label which is which.
...
3. **Flap-prone metric alerts (SNTRY-103).** From `/organizations/{org}/alert-rules/`: `resolveThreshold == null` means the alert relies on the default inverse-of-critical recovery, which flaps near the boundary; a missing `warning` trigger means single-threshold paging with no tiering; a `timeWindow` below `TIMEWINDOW_FLOOR_MIN` ("5" minutes is an example, tune it) on a fixed static threshold fires on transient spikes. `comparisonDelta` (percent-vs-previous mode) or a non-static `detectionType` count as flap resistance. Read the null `resolveThreshold` as the core flap defect; treat the missing warning tier and short window as contributing signals. Two flap modes survive a non-null `resolveThreshold` and a warning tier and so extend this check: (a) a **percent/failure-rate aggregate with no throughput floor** reads 100% on a two-transaction overnight hour — flag percent-aggregate alerts whose `timeWindow` is below `TIMEWINDOW_FLOOR_MIN` or that carry no minimum-volume guard, with org overnight `stats_v2` volume as supporting evidence; and (b) **warning and critical triggers whose actions resolve to the same channel/integration target** double-post every incident — flag identical trigger targets. A `resolveThreshold` set very close to the trigger threshold (0.5 vs 1.0) flaps too, not only a null one.
...
6. **Rule fire-history (SNTRY-106).** The shared data source for the checks below. `lastTriggered` from the rules list gives never-fired for free; `/projects/{org}/{project}/rules/{ruleId}/stats/` returns `[{date,count}]` (~hourly buckets over the ~90-day window) for a measured fire count on any rule a structural check already flagged. Rank the noisiest rules by *measured* fires (a "1,681 fires/90d" number outweighs any structural argument in a customer report) and mark never-fired rules as dead-weight candidates for SNTRY-109. Never scored on its own — it supplies blast-radius evidence that SNTRY-011/014/101/102/107/109 quote.
7. **Chronic-issue re-page join (SNTRY-107).** The flagship noise correlation, joining two surfaces the earlier phases touch separately: top unresolved issues per production project (`/projects/{org}/{proj}/issues/` sorted by events, `substatus: ongoing`, `firstSeen` older than `AGE_FLOOR_DAYS`) × notifying rules whose conditions the issue still matches (frequency/every-event with no age/times-seen/new-issue gate). Emit the re-page ceiling as evidence — `matching_rules × (1440 / frequency_minutes)` pages/day that one permanently-open issue can legally generate (the session's "one 117-day-old issue can page ~96×/day/channel"). This answers *why a channel is unusable*, not *which field is null*.
8. **Name/scope/receiver coherence (SNTRY-108).** Regex the environment claim out of both the rule name (`- Prod`, `- Dev`, `-PP`, `staging`…) and the receiver channel name (`*-pp-*`, `*-prod*`), and require agreement with the rule's actual `environment`. A "Dev"-named, env-null rule posting production events to a PP channel is silent mis-scoping that name-trusting humans never question. SNTRY-102 catches env-null alone; this catches the *lie in the name* and also fires when `environment` is set but contradicts the claim.
9. **Dead-weight rules (SNTRY-109).** Never-fired (per SNTRY-106) **and** (snoozed/muted **or** hair-trigger keyword conditions — `message contains <generic keyword>` with `frequency < FREQ_FLOOR_MIN`). These inflate review surface and coverage counts and delete with zero loss. Distinct from SNTRY-016 (switched off *while crediting coverage*); these are on, dead, and pointless. Check snooze/mute state explicitly alongside `status`.
10. **Ownerless rules (SNTRY-110).** `owner == null` across the issue + metric rule estate. Not SNTRY-017 (delivery routing); this is maintenance accountability — an org where zero rules have owners has no one accountable for tuning, which is *how* an estate rots. Low severity, free read (the field is on the rules list), high explanatory value.

Per-project hygiene (SNTRY-101, 102, 105, 107, 108) batches with the same worklist as Phase 1 on the large path; the org-level metric-alert and stats reads (SNTRY-103, 104) are single cheap calls done once per run. Fire-history (SNTRY-106) adds one GET per *already-flagged* rule — never a blanket per-rule fan-out — and SNTRY-109/110 read fields the rules list already returned. A 401, 403, or 404 on any read here blocks its check with the status as evidence; it is never a clean or passing result. Findings point at `setup-sentry` (`setup-sentry#alert-rule-taxonomy` for SNTRY-101, 102, 103; `setup-sentry#privacy-gates` is unrelated — SNTRY-104 and 105 point at `setup-sentry#quota-spike-protection-and-privacy-sensitive-ingestion`). SNTRY-107, 108, 109, and 110 point at `setup-sentry#alert-rule-remediation-playbook` — the mutation section that tunes noisy tiers, maps rules to environments by receiver channel, deletes dead-weight rules, and sets rule ownership; SNTRY-106 is evidence-only and carries the same playbook pointer for context.
...
2. Recent accepted events exist for that project, counted by numeric project ID from `stats-projects.json` (never a slug search). A mapped project with zero accepted events in the window is a dead project (SNTRY-012). When it does carry volume, that count is the blast radius, not a checkbox: *"checkout maps to project `<id>` which accepted E errors in 14d but has no immediate-tier rule routing to a live receiver — E/day real errors on a critical service reach nobody."* **Split the zero-volume case by `firstEvent` (SNTRY-024)** before writing the SNTRY-012 finding: `firstEvent == null` means the project never received a single event — the SDK was never wired up, and the fix is instrumentation (`setup-sentry#projects`); a real `firstEvent` timestamp with zero *recent* accepted events means the project was instrumented and later went quiet — the fix is investigating the drop-off (a deploy that removed instrumentation, a rotated DSN, a broken ingest path). These are different next actions; do not collapse them into one "dead project" sentence. Command and live evidence in [references/api-checks.md](references/api-checks.md#depth-checks-per-project-quota-attribution-deterministic-source-map-verdicts-vcscode-mapping-integrity-monitor-posture-coverage-split-environment-sprawl-sntry-018-through-sntry-025).
...
| --- | ---: | --- |
| Alert rules and routing | 25 | SNTRY-001, 005, 011, 013, 014, 015, 016, 017, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110 |
| Privacy and data protection | 15 | SNTRY-002, 010 |
| Project configuration | 15 | SNTRY-003, 004, 025 |
| Releases and source context | 15 | SNTRY-006, 009, 020, 021, 022 |
| Service coverage | 15 | SNTRY-012, 024 |
| Monitors | 10 | SNTRY-007, 023 |
| Volume and quota | 5 | SNTRY-008, 018, 019 |

This wave (SNTRY-018 through SNTRY-025) grows each existing category's check denominator; no category is re-weighted. All eight were confirmed against a real Sentry org during this wave — see [references/api-checks.md](references/api-checks.md#depth-checks-per-project-quota-attribution-deterministic-source-map-verdicts-vcscode-mapping-integrity-monitor-posture-coverage-split-environment-sprawl-sntry-018-through-sntry-025) for the observed shapes and live evidence.
...
| SNTRY-008's org-wide drop ratio read as clean while one project is actively losing events | Re-group `stats-projects.json` by numeric project id and apply the same floor per project (SNTRY-018); the org-wide average dilutes a single hot project |
| A rate-limit drop assumed to be a per-key quota problem when it was Spike Protection, or vice versa | Split `stats_v2` by `reason`, not just `outcome` (SNTRY-019); the two causes point at different remediations and neither is provable per-key |
| SNTRY-006's frame-context heuristic treated as the only source of truth on source maps | Read the sampled event's top-level `errors[]` for an explicit, deterministic symbolication-error `type` (SNTRY-020) before falling back to the frame-shape inference |
| A debug-ID artifact bundle's `associations[].release: null` read as "upload missing" | That shape is normal for a debug-ID bundle that resolves independent of any release association; score SNTRY-020's upload-pipeline half on bundle presence, never on whether `release` is populated |
| A junk `origin`/`unknown`-provider repo ignored because SNTRY-009's GitHub-integration check already passed | SNTRY-009 checks whether *an* active integration exists, not every row in `repos.json`; a non-`integrations:*` repo can sit alongside a real one and still receive bound commits (SNTRY-021) |
| A duplicate code-mapping missed because its `stackRoot` strings looked different (`foo/` vs `foo/./`) | Normalize the trailing/leading `./` segment before grouping by `{project, repo}` (SNTRY-022); the strings differ, the resolved root does not |
| Zero cron/uptime monitors treated as nothing to report | When a backend project carries real accepted volume in the same window, record it as an explicit `info` finding (SNTRY-023) — never a silent, uncommented pass |
| A mapped project with zero recent events written up as one "dead project" sentence | Split by `firstEvent`: `null` means never instrumented (wire the SDK); a real timestamp with zero recent volume means a drop-off (investigate the change) — SNTRY-024 |
| An environment-scoped rule read as covering "production" when the project also ingests under a synonym like `prod` | Check the project's environment list for synonym collisions (SNTRY-025) before trusting SNTRY-102/SNTRY-013's environment match; a collision means the rule sees only half the traffic |
| `code-mappings.json`'s `repoId` read via the wrong field name (`repositoryId`, which this API does not return) | Confirm field names against a live response before writing a `jq` selector; a nonexistent field silently returns `null` instead of erroring |
