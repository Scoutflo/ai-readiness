...
**The one branch question (env→branch convention):** once per run, at global/team level — never per service — ask which branch deploys to which environment ("prod deploys from `main`, staging from `release/dev`"). Record the answer as the top-level `env_branch_convention` in `repo-map.json`, propagated to every mapping, with a per-mapping override only where a repo genuinely deviates. If `~/.scoutflo/business_context.md` already answers this, propose that as the default instead of re-asking — but business context is never required and never a prerequisite: this flow owns the question. Never enumerate a repo's branches to "help" the user choose (most branches are stale/temporary noise), and skipping the question is fine — the field is simply omitted. The observed fields need no question at all: `default_branch` arrives free on the resolve/listing call, and `deployed_revision` comes from the workload-level field in `topology-export.json` (ArgoCD/OCI-observed) when present.
