...
- `deployed_revision` (optional) — the exact live commit SHA, mirrored from the **workload-level** `deployed_revision` in `topology-export.json` (where it sits as a sibling of `image`/`image_digest`, per the shared contract) for the workload this mapping's service is deployed as. Only ever a real 40-hex SHA; a branch name never goes here. This is what lets RCA diff the right history at the right commit and name the culprit.
