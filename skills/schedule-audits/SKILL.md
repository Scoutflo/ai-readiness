...
Reports go to `SCOUTFLO_AUDIT_DIR` if the env file exports it, else `./scoutflo-audits` under the entry's `cd` directory (`audit-all` and every audit resolve exactly `${SCOUTFLO_AUDIT_DIR:-./scoutflo-audits}` — a fresh shell reads no config value, so `reports_dir` in `toolkit.yaml` is not a resolution tier here). **The scheduled run and the user's interactive runs must resolve to the same place, or their histories diverge and every scheduled run reports "first run, no delta".** The robust way: export `SCOUTFLO_AUDIT_DIR` (an absolute path) in `$HOME/.scoutflo/env` for the cron run, AND export the same `SCOUTFLO_AUDIT_DIR` in the interactive shell profile (`~/.zshrc`/`~/.bashrc`) so both resolve identically — setting `reports_dir` alone does NOT move where interactive runs write (it only makes `doctor`/`start` print the export line). Relying on the `cd` alone works only while that exact path never changes.
...
| Scheduled cron run reports "first run" though interactive runs have history | The two resolve to different reports dirs. Export `SCOUTFLO_AUDIT_DIR` (absolute) in BOTH `$HOME/.scoutflo/env` (cron) and the interactive shell profile so they match; don't rely on the `cd` matching the interactive launch dir, and don't expect `reports_dir` alone to align them (it only prints the export line, doesn't move interactive writes) |
