...
**Telemetry-scope gate first (LGTM-039).** The configured backends are not guaranteed to monitor the cluster your kubeconfig points at: a central LGTM/VictoriaMetrics stack monitoring workloads on *other* clusters is a common, legitimate deployment. Before scoring any service, run the scope probe in [references/backend-checks.md](references/backend-checks.md) section 12 (compare the metrics backend's cluster-identifying labels and node/namespace inventory against the local cluster's). Three outcomes:

- **Same cluster** — proceed; coverage scoring below applies as written.
- **Different or partially overlapping cluster** — the backends monitor an estate this kubeconfig cannot enumerate. Score coverage only for services whose telemetry scope you can positively establish; mark every local-workload-derived row `blocked` (reason: telemetry backend monitors a different cluster), file it as `LGTM-039` (info) naming both sides, and say plainly in the report that coverage was scoped to the telemetry estate, not the local cluster. **Never file LGTM-030 (or fail LGTM-032/033/034) for a local workload whose telemetry lives on a cluster these backends do not scrape — that is a scope mismatch, not missing coverage.** Likewise, service labels present in telemetry but matching no local workload are *expected* here, not an orphan-label defect.
- **Cannot determine** (no cluster-identifying labels, no `kube_*` inventory metrics) — say so, score what is provable, and mark ambiguous rows `blocked` with that stated reason. Score conservatively but never convert ambiguity into a critical finding.
