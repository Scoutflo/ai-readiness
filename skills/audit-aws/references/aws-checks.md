...
| Aurora/RDS cluster | at least one reader in a different Availability Zone from the writer, resolved by joining cluster members to `rds.json` | cluster `backup_retention_days` | `not-in-scope`: Aurora storage grows automatically | `AuroraReplicaLag` per reader or `AuroraReplicaLagMaximum` at cluster level, using the dimensions the live metric exposes | `AWS/RDS`; use `DBInstanceIdentifier` for instance metrics and `DBClusterIdentifier` only for cluster metrics |
