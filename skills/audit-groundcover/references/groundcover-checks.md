...
- **Confirmed vs capability-gated.** The monitor list, workflows, and recurring silences are confirmed in groundcover's docs. On SaaS the per-monitor **config + runtime state** (severity, isPaused, state, silenced, fullySilenced, pendingFor, lastEvaluationError, alertingCount, **and `noDataState` + `executionErrorState`** — all confirmed live on a real 200 response) comes from `POST /api/monitors/summary/query`. The per-monitor `GET /api/monitors/{uuid}` returns **YAML** (never pipe it to jq) and its config surface is thin — no `notificationSettings`/`autoResolve`/`customResolveThreshold` — so the fields it alone carries are best-effort enrichment (and `model.thresholds` there is an array). When `summary/query` is not 200, or a config field is genuinely absent, mark the dependent checks (GC-002/003/004/005/010-013, GC-022/023) `not-in-scope` with that reason. Never guess a monitor's live state.
...
if [ "$RS_CODE" = "200" ]; then
  jq -c 'if type=="array" then . else (.monitors // .results // .summaries // []) end
      ...
      | {uuid, title, severity, isPaused, state,
         ...
         fullySilenced: (.fullySilenced // null),
         pendingFor: (.interval.for // null),
         lastEvaluationError: (.lastEvaluationError // null),
         alertingCount: (.alertingCount // null),
         noDataState: (.noDataState // null),
         executionErrorState: (.executionErrorState // null)}' "${RAW_DIR}/monitor-state.json" \
...
# notificationSettings.*) live only in the per-monitor GET, which is YAML — so parse it with a YAML
# tool (yq) into JSON first, NEVER pipe it to jq. (`noDataState`/`executionErrorState` DO come from
# summary/query — confirmed live — and are already in the projection above; the YAML read below only
# re-enriches them, so GC-004/GC-005 stay assessable on a yq-less machine.) model.thresholds
...
RUN_DATE="$(date -u +%Y-%m-%d)"
RAW_DIR="${SCOUTFLO_AUDIT_DIR:-./scoutflo-audits}/groundcover/${RUN_DATE}/raw"
# GC-002/003 key off config fields (customResolveThreshold, autoResolve) that /api/monitors/summary/query
# does NOT carry — they exist only when the per-monitor YAML config was parsed in section 4.
# GC-004/005 key off noDataState/executionErrorState, which summary/query DOES carry (confirmed live;
# the section-4 projection retains them), so those two are assessable even without yq. When a field is
# null on EVERY monitor it was not exposed this run, so its check is
# not-in-scope-from-this-endpoint, never a blanket finding.
...
if [ "$(cat "${RAW_DIR}/monitor-state.http" 2>/dev/null)" = "200" ]; then
  ...
  jq '{stuck_firing: [.. | objects | select(((.state? // "") | test("^(alerting|firing)$"; "i")) and (.lastResolved? == null))
