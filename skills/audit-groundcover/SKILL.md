...
- **GC-014 (duplicate/overlapping monitors — verify-pending, new)** — section 6.1. groundcover has no native dedup, so N monitors on the same `(namespace, workloadName, type)` routing to the same destination fire N simultaneous pages for one condition. Report the group's `duplicate_count` and named monitors, multiplied by GC-010's cadence for redundant pages/day. **Gated, positive path still unproven:** it computes only when the dedup-key fields were captured (section 4); otherwise `not-in-scope-from-this-endpoint`. A first live run confirmed the gate fires correctly (the join fields were 0/N on that estate, so the check honestly stood down); a duplicate count has still never been computed on populated keys, so the positive path stays verify-pending.
...
| Runtime monitor state assumed readable | The monitor-summary endpoint (`POST /api/monitors/summary/query`) is confirmed live on SaaS (200 + per-monitor runtime state incl. `state`/`noDataState`/`executionErrorState`), but still probe once per run and mark GC-022/GC-023 `not-in-scope` if it is absent on this host — never guess a monitor's live state |
