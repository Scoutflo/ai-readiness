...
- **DO-073 (destination-set consistency).** Unlike DO-070 to DO-072, this is not about a control DO Monitoring lacks — the destination fields are fully readable. Within each alerting surface (policies grouped by `type`), compute the mode destination signature (`emails_count`, `slack_count`) that most enabled siblings share, and flag any policy whose signature deviates — one uptime-adjacent DB policy emails-only while every sibling of the same metric type has email plus Slack. Deterministic and free of channel-correctness judgment (that stays with DO-003): it only asks whether siblings watching the same signal are wired the same way.

All four reads are single cheap `jq` passes over the existing inventory, done once per run; they do not batch per app on the large path. As everywhere in this audit, a `401`/`403` while refreshing the policy list is an auth-scope finding that blocks these checks, never a clean or passing result.

## Phase 4: Uptime and availability (DO-010 to DO-017)

Commands in section 6. Every active public hostname has an uptime check (`DO-010`); each check carries a down alert (`DO-011`), a latency alert (`DO-012`), and an SSL-expiry alert (`DO-013`); multi-region checks where a single region would mask regional failure (`DO-014`); no check monitors a dead, archived, or migrated target (`DO-015`); the live TLS certificate on each monitored HTTPS hostname is not already within the expiry window (`DO-016`); and no hostname with zero uptime coverage is answering a live `5xx`/transport failure right now (`DO-017`, high — the flagship join of DO-010's uncovered set with the section 4 live probe). Probe every check target live and capture the status code as evidence. Deepen DO-010 past "hostname X has no uptime check" to the outage that goes undetected — DO-010 is the detection leg of the silent-outage cascade below, and `DO-017` is what turns that detection gap from hypothetical into a live-incident finding the moment the uncovered hostname is actually failing.

> **Live-verified (read-only).** `DO-016` (live-cert read) was run against a live DigitalOcean account: the passive TLS handshake against a real App Platform app hostname returned a valid certificate expiry date, proving the openssl mechanic end to end. Its handshake stays passive/read-only; a connect failure / `000` or handshake error is a **BLOCKED** result cross-referenced to DO-010 (no check at all) and DO-060 (the host may have moved off DO), never a fabricated high fail — see section 6.1. `DO-017`'s join (section 6.0.1) was also run against a live account: real uncovered hostnames answered `503` at probe time, two of them belonging to apps whose `active_deployment.phase` read `ACTIVE` at the same moment — the exact "the platform stays silent, only the probe catches it" verdict, observed on real output.
...
- ❌ `Uptime fail (DO-010 only): billing-internal has no uptime check.`
- ✅ `Uptime fail (DO-010, escalated to DO-017): billing-internal has no uptime check AND is answering 503 right now — this is not a gap, it is an undetected outage in progress; active_deployment.phase still reads ACTIVE, affected: billing-internal.`

## Phase 5: App Platform coverage (DO-020 to DO-033)

Commands in section 7. Per active app: deployment lifecycle alerts at least for failed and live (`DO-020`), domain lifecycle alerts (`DO-021`), CPU and memory alerts (`DO-022`), restart-count alert (`DO-023`), request-rate and p95-duration alerts backed by an observed baseline rather than guessed thresholds (`DO-024`), alert rule names recorded exactly as the API returns them, noting any doc-versus-API enum mismatch for future automation (`DO-025`, info), and — where app names encode environment — prod carries every enabled alert rule its PP sibling carries (`DO-026`, high, section 7.1: a rule present in PP but dropped from prod is a coverage regression that shipped without its alert; `not-in-scope` for apps with no recognized environment sibling).
...
| --- | ---: | --- |
| Alert routing and delivery | 20 | DO-001 to DO-005, DO-070 to DO-073 |
| App Platform alert coverage | 20 | DO-020 to DO-026 |
| Managed databases | 20 | DO-040 to DO-048 |
| Uptime and availability | 15 | DO-010 to DO-017 |
...
Weights sum to 100 and are unchanged by the three newer checks (`DO-017`, `DO-026`, `DO-073`): each folds into its existing category — `DO-017` grows Uptime and availability, `DO-026` grows App Platform alert coverage, `DO-073` grows Alert routing and delivery — growing that category's denominator, never re-normalizing a weight.
...
| An uncovered hostname filed only as a DO-010 coverage gap while it is actively answering 5xx | Always run the DO-017 live probe against DO-010's uncovered set; a currently-failing uncovered hostname is a live incident, not a monitoring backlog item |
| Environment base-name stripping matched only the prod suffix, leaving the PP app's base name un-stripped so the pair never grouped | Strip EITHER the prod or the PP/staging suffix when computing an app's base name for pairing (test both regressed apps' base names are byte-identical), even though the env classification itself still tests each suffix class separately |
| DO-073's mode signature computed as a strict majority instead of "the most common shape" | A 2-vs-2 split within one surface has no strict majority; take the largest group by `group_by | max_by(length)` and flag every other group as deviant, rather than requiring `> 50%` |
