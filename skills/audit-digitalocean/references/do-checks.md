...
> **Reads confirmed live; emit-path unobserved.** These two checks' reads have now been run read-only against a real DO tenant with App Platform apps: `active_deployment.phase` and `in_progress_deployment` are present and readable on every app (observed `ACTIVE` / `null` on a healthy estate). The **emit** conditions (`ERROR`/`CANCELED`, a wedged in-progress phase) were not observed, so the emit path stays live-unobserved until a failed/wedged deployment is actually seen — never fabricate one. The reads below are ordinary `doctl apps list`/`get`/`list-deployments` calls (read-only).
