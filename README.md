...
| See which service uses which database, cache, or queue (AWS / DigitalOcean / Azure / GCP — or via your APM on any estate) | `/scoutflo:map-topology` — Cloud Mode proposes evidence-backed connections; you confirm them in one batch |
...
- **Onboarding got sturdier for big, messy estates.** If your kubeconfig has dozens of contexts — some deleted, some duplicates of the same cluster — connect can now triage them read-only: it dedups by API server, checks which are actually reachable (with a bounded timeout so a dead one can't hang it), and offers only the **live, distinct** clusters, so you never point an audit at a stale or duplicate context. And if your secrets live in **Vault** (or another fetcher) rather than as pasted values, `connect` can store `export TOKEN="$(vault kv get …)"` so the token resolves fresh on every run instead of a static copy sitting on disk — with a clear "trusted fetch only" safety note.

- **`audit-lgtm` now covers multiple clusters/environments in one config, one run.** If your logs, metrics, and traces stores live on different clusters (a prod stack and a pre-prod stack, say), make `lgtm:` a **list** of stack entries — each with its own cluster context and store URLs — instead of hand-splitting into one config file per stack. The audit runs each stack in turn and writes its report under `lgtm/<label>/<date>/`; `/scoutflo:doctor` validates each stack and probes its stores. A single stack is unchanged (zero migration). This was a real customer blocker — a monitoring fleet spread across clusters no longer means N configs and N runs.

- **Adding a credential is now one safe move — and doctor pinpoints the fix when one won't load.** `/scoutflo:connect` stores each token with a shipped writer that escapes the value correctly (a `$`, a quote, or a backtick in a token is kept literal), replaces that variable's line instead of appending a duplicate on a re-run, keeps the store `chmod 600`, and never echoes the secret — so a wrapped paste, a special character, or a "command not found" can no longer leave the store empty or leak a token. When a token *is* set but an audit still can't see it, `/scoutflo:doctor` now names the precise cause — a line missing its `export ` prefix, a malformed line, or a value exported only in your shell (which the plugin, reading the file, never sees) — and the one command that fixes it. And connect only sets up multiple environments when your own answers show more than one, so a single estate is never pushed into per-environment files.

- **connect now detects the cloud logins you already have.** A read-only sweep at the start of `/scoutflo:connect` spots existing `az` / `gcloud` / AWS-profile / `doctl` sessions and offers the matching config block — so a valid cloud login can no longer sit invisible to the toolkit just because its block was never written. Detected logins are offers to confirm, never auto-written.

- **Your inventory now reads by environment — and there's a service map.** Every audit's inventory renders a **By-environment** view (per-environment object counts, and *environment twins* pairing the same base name across prod / pre-prod / staging — a thing that exists in only one environment is flagged, so a pre-prod that should mirror prod can't look empty just because it has fewer connections). map-topology writes the same complete **server + datastore catalog per environment** into `topology.md` (a server is listed whether or not a connection was found), plus a **service map**: a Mermaid diagram with directional arrows labelled by connection type + evidence, datastore cylinders showing `engine · port`, and nodes coloured by environment — renders on GitHub/Obsidian and exports to PNG.
- **Cloud Mode now reads your network's own story.** Where flow logs exist (AWS, GCP, Azure), map-topology observes real connections — who talked to which database, on which port — with zero secret or config access; where they don't, it tells you the one setting that would unlock them. Your metrics store also counts now: Tempo service-graph data alone yields service names and call edges. And every permission denial answers with what CAN be mapped, the smallest unlock, and the workaround — never a dead end.
- **map-topology Cloud Mode across AWS, DigitalOcean, Azure, and GCP: the resources behind your services.** map-topology now also maps databases, caches, queues, topics, and buckets, and proposes the service→resource connections it can prove — each with its evidence (what the config declares, what identity permits, what the network allows, what traffic shows). You review them in batches (one bulk confirm for the strong ones, real questions for the weak ones), re-runs never re-ask what you answered, and it works at three access levels, down to a zero-access script your own team can run. It never reads or requests secret values at any level. An APM overlay (New Relic, Tempo) adds observed database edges on any estate — including a pure Kubernetes one.
- **No Kubernetes required for topology.** map-topology routes to the best source you actually have: your cluster when you have one, otherwise cloud inventory + APM, otherwise a guided capture — never a dead end.
