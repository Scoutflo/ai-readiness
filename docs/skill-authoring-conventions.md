...
- **Read-only discipline is unchanged and lane-bound.** In an `audit-*` skill, call only read-only MCP tools (list/get/describe/query). Never call an MCP tool that creates, mutates, silences, acknowledges, or deletes, however small; if a server only exposes a mutating tool for what you need, use the read-only CLI/HTTP path instead. `setup-*` skills follow their announce/confirm/execute/verify loop for an MCP write exactly as for a CLI write.
...
- **Live-safety is unchanged.** Confirm the MCP tool is pointed at the same target the config names (same cluster, account, org, host) before trusting it, the same identity check the CLI path does.
