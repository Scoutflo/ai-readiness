...
LGTM and the VictoriaMetrics family (Loki, Tempo, Mimir, VictoriaMetrics/Logs/Traces, Alertmanager, vmalert), Grafana, Sentry, PagerDuty, Datadog, ELK/Kibana, JSM Operations, Zenduty, groundcover, Prometheus/Alertmanager alert routing, DigitalOcean, Google Cloud, and AWS. Connect only the ones you use; `/scoutflo:start` lists the full catalog.
