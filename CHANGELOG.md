...
- **Deterministic disposition skeleton** (`lib/migration-plan.sh`, zero provider calls): artifact-first over the source audit's `inventory.json` + `findings.json` (+ optional `fatigue-signals.json`) and the target's inventory, every source object gets ONE evidence-cited disposition — `migrate` / `fix-then-migrate` / `drop-candidate` / `already-covered` / `no-equivalent` — so the audits' proof (never-evaluated, duplicates, placeholder handles, measured never-fired/dead-end) turns lift-and-shift into shed-the-noise, as **proposals pending confirmation, never silent drops**. Target unreachable ⇒ honest `source-only` mode (no `already-covered` claims possible).
- **`references/datadog-to-signoz.md` — the pair catalog**, grounded in a two-agent primary-source research pass (Datadog official API-client models + docs; SigNoz docs + source), VERIFIED/UNVERIFIED-flagged throughout, and citing SigNoz's own official migrate-from-Datadog guide set. Full equivalence attempted for **every** object kind: the 23-type monitor matrix (incl. DD warn/critical/recovery → SigNoz v2alpha1 multi-thresholds + `recoveryTarget` + per-threshold channels — a better shape than expected), notification handles → channels/routing policies (with the handle-enumerability matrix), the 41-widget dashboard matrix (v2/Perses import path; the Cloud-paid LLM tool stated as Cloud-paid), SLO reconstruction recipes (burn-rate pair = **two** rules — multi-window-in-one verified impossible), synthetics (no native equivalent → `httpcheck` receiver + uptime template), downtimes → planned maintenance (RRULE gaps named), log pipelines (22 processors mapped), and the metrics/tags/agent dual-write bridge (`DD_ADDITIONAL_ENDPOINTS` → OTel datadogreceiver, alpha, metrics-scope). Untranslatable parts are classed `manual` and **named** — never silently auto-translated.
- **`report-standard/check-migration-plan.sh` — the honesty gate** (hard gate before rendering): disposition enum + reasons, **no evidence-free drop/fix ever**, ghost-finding-id cross-check against this run's findings, `already-covered` forbidden in source-only mode, equivalence enum, real gap alternatives, totals reconciliation, and the structural lock `cutover.historical_telemetry == "does-not-transfer"` (config carries; **history never does** — SigNoz's own docs cited; continuity = dual-write parallel run + the audit-parity sunset gate: re-run both audits + correlation, sunset only on coverage parity with no true-gap regressions).
- **Rendered deliverable**: `render-report-viz.sh` gains `migration-plan` (markdown) + `migration-plan-html` (self-contained dashboard) modes — disposition tiles, per-disposition tables with evidence, gaps with alternatives, the cutover section. Cataloged in README + `/scoutflo:start`; contract **C19** in `docs/CONTRACTS.md`; 3 pressure scenarios.

**Verification:** all four repo gates green; `test-migration-plan.sh` **31/0** (disposition rules + precedence, source-only honesty, unsupported-pair refusal, validator red paths incl. ghost-evidence — which caught a real jq `$var | index(.)` rebinding bug in the validator during build); `test-report-viz.sh` **+Test 18**. The cross-block gate caught two real unbound-`RUN_DATE` blocks in the SKILL during build. **Live smoke:** the engine chain is proven hermetically end-to-end (fixtures → skeleton → validator → md/html); no Datadog tenant was read from this session (no standing authorization) — the first live run is the customer session the skill was promised for, per the maintainer-smoke handoff rule. Read-only throughout; the library makes zero provider calls; non-scored.
...
- **P2 — `fatigue-signals.json` is now validated (`report-standard/check-fatigue-signals.sh` + `tests/test-check-fatigue-signals.sh`, 15 cases).** The measured tier's input is produced by the model-driven fire-history collection lane — the highest hallucination risk — and previously fed AF-004/005/006 with no gate. The validator fails closed on a malformed schema, a wrong field type (incl. the `//`-bool trap), an `off_hours_fires > fires` impossibility, a reasonless `verify-pending`, or a `source_finding_ids` that cites a finding **absent from this run** (the never-fabricate cross-check). The collection lane now runs it (SKILL step 3b) before the roll-up.
- **P3 — the executive one-pager stamps a triage run.** `render-report-viz.sh exec-summary` now detects any `scope: "triage"` findings and prints **"TRIAGE — subset, not a full assessment,"** and the grade never reads as a bare "HEALTHY" from a subset — closing a false-confidence gap where a fast partial pass looked identical to a full audit.
- **P4 — AF-004 reachability reports its coverage denominator.** The lib now carries `objects_seen` alongside `measured_objects`; when routing was resolved for only a subset, the report says "routing resolved for M of T objects seen," so "N cannot reach a human" never implies the whole estate from a partial read.
- **P1 — the measured tier is honestly labeled unverified-until-live.** `fire-history-reads.md` gains a per-provider **verification ledger + live-verification checklist** and a `provider_coverage[].verification` field (`spec-only` until a first live run confirms the flagged fields, then `live-verified`); the report surfaces a per-provider coverage line so a `spec-only` provider is never presented as battle-tested. The actual per-provider live run remains the standing maintainer action (added to the self-test **PLAYBOOK** § 4b) — it needs an authorized estate; this release makes the debt explicit and deterministic to close.
- **Latent bug fixed:** the alert-fatigue markdown report exited early on zero config-noise findings, **hiding the measured tier** even when fire-history data existed — it now renders the measured tier standalone.
- **Documentation refresh:** README (new "Alert noise & fatigue, triage, and the executive one-pager" section), FAQ (three-tier answer + triage + exec one-pager Qs), token-costs (honest triage note — reads less, no wall-time claim), and CONTRACTS **C17** (validator guard, verification/`spec-only`, AF-004 coverage). Self-test gains 6 locks for the measured tier / validator / triage / P4 denominator; PLAYBOOK gains the fire-history live-verification + triage steps.

**Verification:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 32 suites, `plugin validate --strict`) + self-test lock. New pressure scenario (measured-tier honesty + validation). `test-report-viz` +Test 17 (triage stamp + AF-004 coverage), `test-check-fatigue-signals` 15/0. All new shell verified under bash 3.2 (macOS `/bin/sh`) with zero control bytes. Read-only throughout; the roll-up library still makes zero provider calls.

## 0.1.184

**Fast triage mode + an executive one-pager, and fixes for what a live large-estate run surfaced.** A full deep audit of a ~700-object AWS estate ran 33 min / 150k tokens — right for a scheduled run, wrong for a 30-minute session. This adds a fast, honest first pass and a top-of-report exec view, plus targeted fixes for issues seen in that run. Design grounded in a cited research pass (AWS Trusted Advisor Priority / Security Hub / Config, Google SRE golden signals + mitigate-before-diagnose, CVSS + Security Hub severity≠$/criticality, PagerDuty SEV, CIS L1/L2).

- **`report-standard/triage-mode.md` (new) + `SCOUTFLO_TRIAGE` contract:** a fast pass that runs each provider's **high-signal, cheap-to-read subset** (golden-signal proxies, alarms firing now, dead-end routing, data-loss/exposure), at the smallest scope, preferring **bulk reads** (`describe-alarms --state-value ALARM`, `get-metric-data` batched ≤500, ResourceGroupsTagging/Config aggregator) over per-resource fan-out — and never backgrounds a sweep. Per-provider triage subset table included. `audit-all` gains a documented triage fast path; a triage `findings.json` is marked `scope: "triage"` so a subset is never reported as a full all-clear.
- **`render-report-viz.sh exec-summary` (new mode) — the executive one-pager**, now the top section of the `audit-all` report: a **posture grade** (AT RISK / NEEDS WORK / FAIR / HEALTHY) + severity counts, the **top 5–7 worst findings across the estate** each as *what · where (blast radius) · $ · the fix*, the measured **alerting-reachability** headline (AF-004), and the single top provider-native **$ lever**. Ranked **severity-first → recoverable points → $ as an in-band tiebreaker — never a blended cross-domain score**, and a big $ figure never promotes a lesser finding above a critical one (CVSS / Security Hub keep severity separate from $/criticality).
- **Observed-issue fixes:** (a) *background-wait error* — the shared scope-checkpoint now mandates running scoped sweeps **inline, never backgrounding/scheduling a wakeup** (the `prompt is required` scheduler error that leaked into a live run); (b) *fire-history ↔ audit-aws overlap* — the fatigue fire-history lane now **reuses the config audit's inventory** (one enumeration, pull only the history delta, cite back) instead of re-listing; (c) *correlation ↔ AF-004 double-vocabulary* — the report now cross-references correlation's config-derived "true gaps" with AF-004's measured reachability instead of restating; (d) *cost enablement* — new **COST-AWS-025** surfaces "Compute Optimizer not enrolled → rightsizing blind; enrollment is free" as one actionable finding (not seven silent `excluded` reasons), and keeps the RI unused-commitment waste (COST-AWS-012) prominent.
- **Deferred (honest):** true incremental/cached re-runs (skip unchanged resources) — a real want, but result-caching correctness is too risky to ship safely in this pass; noted for a dedicated design.

**Verification:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 31 suites, `plugin validate --strict`) + self-test. `test-report-viz.sh` **+Test 16** (exec-summary posture grade + severity-first ranking with $ never promoting a low finding + reachability headline + empty-estate degrade). Renderer verified under bash 3.2 (macOS `/bin/sh`), zero NUL bytes. Read-only throughout; exec-summary computes only from artifacts already on disk.

## 0.1.183

**`alert-fatigue` upgraded from aggregator → analyst: a read-only fire-history collection lane + a measured analysis tier.** The roll-up used to only re-organize the config audits' findings (fast, but shallow — it re-listed structural noise). Now the skill owns the definition of fatigue and collects the **measured tier** itself: per configured provider it makes **read-only fire-history API calls** — the reads the config audits skip — and analyzes them into measured findings no config scan can produce. Grounded in a deep, cited API-surface research pass across every provider (AWS/Datadog/Grafana + Prometheus/Alertmanager/SigNoz + the incident-feed tools), with VERIFIED/UNVERIFIED flags preserved.

- **New fire-history collection lane (in the skill; the roll-up library stays zero-provider-call):** read-only reads per provider write `fatigue-signals.json` (`scoutflo-fatigue-signals/v1`: per-object `fires`/`flapping`/`stuck`/`off_hours_fires`/`reaches_human`, `provider_coverage[]`, and an optional live `incident_feed`). Full per-provider read spec + derivations + the output contract in the new **`references/fire-history-reads.md`**: CloudWatch `DescribeAlarmHistory` + SNS subscriber check, Datadog Events + monitor state, Grafana annotations/state-history, Prometheus `ALERTS`/`ALERTS_FOR_STATE` PromQL, Alertmanager notification counters + silences, SigNoz rule-history routes, and PagerDuty/incident.io/Opsgenie incident feeds. Every call is a GET / read-by-POST — **never mutates**; off-hours is always **derived** from a timestamp (no native field anywhere).
- **New measured analysis items (`alert-fatigue.json`):** `AF-004` reachability ("**N of M alerting objects cannot reach a human by construction**" — dead-end SNS / zero-workflow / no-channel, grouped by reason), `AF-005` measured volume & flapping with **top offenders ranked by fatigue impact** (fires weighted by flapping / off-hours share / dead-end routing), `AF-006` chronic/stuck (firing continuously for a long time), `AF-007` fatigue anti-pattern histogram (dead-end / chronic-stuck / flap-prone / un-gated-repage / ownerless / decoration-or-dead-weight). `AF-003` now computes the alert-to-incident ratio + MTTA/MTTR/%-actionable from a **live incident feed** when present (operator `fatigue.json` is the fallback).
- **Never fabricates a measured number:** a provider that can't expose history is `verify-pending` in `provider_coverage[]`; with no `fatigue-signals.json`, AF-004/005/006 are `not-in-scope` and the config tier still reports fully. The report + HTML dashboard render the measured tier (reachability headline, fatigue-impact-ranked offenders table, chronic-stuck, anti-pattern histogram, two new metric tiles) and say plainly where a tier wasn't collected.

**Verification:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 31 suites, `plugin validate --strict`) + self-test. `test-alert-fatigue.sh` **33/0** (adds the measured tier + never-fabricate + live-feed-preferred assertions); `test-report-viz.sh` gains **Test 15** (renders the measured tier + degrades honestly). Two research agents nailed the read-only API surfaces (verified/unverified-flagged). **Live-smoke:** the analysis engine is proven hermetically end-to-end on synthetic fire-history signals (reachability, fatigue-impact ranking, chronic, histogram, live-feed ratio); the per-provider fire-history *reads* are documented read-only against primary API docs and are the maintainer/standalone live-smoke against an authorized estate (a fixed bash-3.2 NUL-byte parse bug + a jq `//` coercion bug were caught in-build). Read-only throughout; the roll-up library makes zero provider calls.

## 0.1.182

**Deep alerting-hygiene research pass — our checks stress-tested against the full literature; the small real gaps closed and the grounding hardened with peer-reviewed anchors.** A four-front research sweep (Prometheus/Alertmanager incl. pint + promtool, Grafana + Datadog docs, Sentry + SigNoz docs, and academic/cross-vendor incl. peer-reviewed papers) checked what a *clean* alerting setup requires, the named noise/fatigue anti-patterns, and how to detect gaps in an *existing* setup via read-only inspection. The headline result: **the research overwhelmingly validates our existing coverage** — nearly every documented vendor best-practice and anti-pattern already maps to a shipped check, independently corroborated by Cloudflare's **pint** linter and the SRE burn-rate canon. Only a few genuine, non-duplicate gaps existed; this closes them and deepens the grounding, without inventing unverifiable checks to look busy.

- **audit-datadog `DD-039` (new, Monitor noise):** `require_full_window` posture on metric monitors. Flags the mechanically-certain half from config alone — `require_full_window == false` on a dense metric evaluates partial windows and flaps at the window edge — and treats the sparse-metric inverse (`true` → windows never fill → skipped evals → silent gaps) as a cadence-verify follow-up, never a fabricated fail. A missing field is the type default `true`. Same monitor pull, no new API call. Category weight unchanged.
- **audit-signoz SIG-042 + SIG-040 deepened (no new IDs):** SIG-042 gains a fourth quality axis — SigNoz's `condition.matchType` defaults to the flap-prone **`at least once`** (fires on one transient spike); a windowed match-type (`on average`/`all the times`) is the SigNoz equivalent of a Prometheus `for:` dwell. SIG-040 now distinguishes a **forgotten disable** (a `disabled` rule with no covering planned-maintenance window = a silent blind spot) from a deliberate maintenance suppression. Both are config-tier reads over the existing endpoints; verify-pending-live where SigNoz is unreachable.
- **`skills/alert-fatigue/references/methodology.md` hardened with peer-reviewed anchors (§6):** Zhao et al. ICSE-SEIP 2020 (alert-storm, **>98% reducible**, DOI 10.1145/3377813.3381363) as the defensible correlation/compression citation; Chen et al. ICSE-SEIP 2019 (incident reassignment **4.11–91.58%**, **10.16× time cost**) anchoring the ownership/routing checks; Sendelbach & Funk 2013 / Cvach 2012 (72–99% false-alarm rate → the transferable law that per-rule *actionability*, not raw volume, drives missed real alerts). Adds a **source-tier labeling discipline** (`primary-study` › `industry-canon` › `vendor-methodology` › `vendor-survey/folklore`), pint/promtool as external corroboration of the config-tier checks, the exact vendor defaults to key deviations off, and reconfirmed folklore flags (1:1 ratio is a target; population MTTA/MTTR/%-noise/%-off-hours are vendor-survey; SigNoz/Sentry have no native burn-rate SLO alert).

**Verification:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 31 suites, `plugin validate --strict`) + self-test lock. Two new pressure scenarios (DD-039 two-sided honesty; SigNoz match-type flap + forgotten-disable verify-pending). **DD-039 logic proven by a hermetic monitor-fixture smoke** — it flags only an *explicit* `require_full_window == false` on a `metric alert` (a missing field = the `true` default is correctly NOT flagged, and non-metric types are ignored); the smoke caught and fixed a real jq `//`-coercion bug where `.options.require_full_window // true` silently coerced the `false` we want to flag into `true`. A live-tenant Datadog run was **not performed this session** (the configured tenant's scope was not confirmed for the current turn); DD-039 is a pure config-selection read, so the fixture smoke is the load-bearing proof, and a maintainer live run against an authorized org is the standing follow-up. The SigNoz deepenings are **verify-pending-live** (in-cluster instance not reachable — honestly flagged, consistent with the existing SIG-040/042 posture). Read-only throughout; no schema change; category weights unchanged.

## 0.1.181

**`alert-fatigue` now produces a pretty, structured report that shows every problem AND its exact fix — the deliverable a user reads, not raw JSON.** The 0.1.180 two-mode work computed the right data but emitted only `alert-fatigue.json`; a customer prompt could get a shallow pass from any LLM. This ships the depth-and-presentation moat: a deterministic renderer that turns the roll-up into a report at a depth and accuracy a cold prompt won't reach on run one. Design informed by a market pass on how the leading tools present alert-noise/operational reviews (PagerDuty Operational Reviews "loudest service"/interruption split, Datadog "noisiest monitors", Grafana OnCall insights, Google SRE precision/recall, BigPanda/Moogsoft compression) — steal the worst-first "top offenders" table and the per-finding problem→fix pairing; reject the un-reconcilable "N% noise reduction" headline.

- **New `render-report-viz.sh` modes `alert-fatigue` (markdown) + `alert-fatigue-html` (dashboard).** Both render `alert-fatigue.json` and **join each cited noise finding back to its home `findings.json`** for the fix text (`recommendation` + `remediation` pointer) — so the roll-up stays a lean citation while the report shows the exact fix, and the two can never disagree with the numbers. Read-only; renders only structured fields (never a secret / raw command output); HTML is self-contained and asset-free.
- **The report layout:** an at-a-glance line (noise findings / tools / storms / ratio), the honest three-tier framing, a **worst-first "top offenders" list where every finding shows problem → where → why it matters → the exact fix**, where the noise concentrates by tool, the cross-source storms (the cross-tool wedge no single vendor sees), the alert-to-incident ratio (computed, or an explicit **"not measured this run — needs an incident feed"** block listing the gated metrics rather than fabricating them), and the cited "what good looks like" benchmarks.
- **Wired into both modes:** standalone `alert-fatigue` writes `alert-fatigue-report.md` + `alert-fatigue-report.html`; `/scoutflo:audit-all` Phase 3.6 writes the HTML dashboard and its report §7 renders the same markdown section (replacing the old bare one-line `jq` summary). Contract **C17** updated (`docs/CONTRACTS.md`); non-scored / cites-source-IDs / re-scores-nothing discipline unchanged.

**Verification:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 31/0 suites — `test-report-viz.sh` gains Tests 12–14: the join to the exact fix, worst-first ordering, exclusion of non-cited findings, self-contained/asset-free HTML, and missing/zero-noise degrade; `plugin validate --strict`) + self-test lock. New pressure scenario (the deliverable is a rendered problem→fix report, not raw JSON). Rendered output verified visually (HTML dashboard screenshotted) and against a synthetic multi-tool fixture. Read-only throughout; the roll-up lib and its schema are unchanged.
...
- **Cited "what good looks like" benchmarks** the checks score against: every-page-actionable, **≤2 incidents/12h**, on-call ≤25%, multi-window burn-rate (14.4/6/1; short ≈ 1/12 long, ≤48h), **1:1 alert-to-incident target**, ~30-min grouping / 24h-dedup defaults, the **70–85% AIOps compression band with the over-compression caveat**, and off-hours bands — plus verified stats (SRE "144 alerts/day and still meet SLO"; Drew 2014 "2.5M alarms, 88.8% false"; Joint Commission "85–99%") vs explicitly-flagged vendor/folklore (no fabricated compression ratio / off-hours %). Full principles, anti-patterns, per-tier auditor checklist, vendor technique catalog + citations in the new `references/methodology.md`.

**Verification:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 31/0 — the roll-up lib and its test are unchanged, `plugin validate --strict`) + self-test lock. New pressure scenario (standalone on one integration must produce the honest tiers, never fabricate the ratio). **Live-smoke:** the roll-up lib is unchanged/tested and the per-provider alerting-lane checks it invokes are each already live-verified in their own releases; a full standalone end-to-end run against a live provider (e.g. Datadog/Sentry) is the maintainer smoke. Read-only throughout.
...
- **audit-signoz — new SIG-070 "metric cardinality health"** (scored, high; grows the ClickHouse-health category denominator, weights unchanged, still sum to 100). For the top metrics by sample volume it reads distinct label-value counts from the metrics store and classifies each label: **UNBOUNDED** (`url.full`, `db.statement`, ephemeral ports…), **IDENTIFIER** (`trace_id`, `user.id`, `session.id` — never valid as a metric label), **ACCUMULATING** (`k8s.pod.uid` — churn, not live series), **HIGH-but-bounded** (`valueCount > ~100` and unused by any dashboard/alert), **DEPLOYMENT_DEPENDENT**. The fix names the OTel Collector `metricstransform aggregate_labels` (merge series — never `delete_key`, which doesn't cut sample count). **Do-not-drop guard:** `system.*`/`k8s.*`/`signoz_*` families are never called "safe to drop" (they power the Hosts/Kubernetes/APM pages) — the finding is "reduce the *label* cardinality," and trace sampling is never suggested as a cost lever. ClickHouse-lane check: `not-in-scope`/renormalized when the deep lane isn't configured (the SIG-030/060/061 path).
- **audit-signoz — ClickHouse read-path optimization notes** folded into the deep lane: `resource_fingerprint GLOBAL IN` (plain `IN`/`JOIN` fails under `distributed_product_mode='deny'`), `lower(body)` skip-index engagement rules, the mandatory `ts_bucket_start − 1800` companion time filter, and the query-API "formula top-N applied before formula evaluation" truncation trap.
- **audit-cost — new `references/signoz-cost.md` (`COST-SIG-001..004`)**, cloning the proven never-invent Datadog-cost template: per-signal ingest volume from the SigNoz Cost Meter (`source:meter`, hourly sum, `partial` buckets excluded), top metrics by samples (histogram `.bucket` callout), non-prod env >40%-of-volume, and high-cardinality label contributors (cross-refs SIG-070). Every finding is a **presence fact** — `estimated_monthly_savings_usd` and `estimated_monthly_cost_usd` are `null` because SigNoz publishes no per-signal cost; volume (GB / samples) is the billable dimension, no dollars invented. Wired into the audit-cost provider table + doctor matrix; `signoz` added to the cost-schema provider slugs.

**Verification:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 30/0, `plugin validate --strict`) + self-test lock. New pressure scenario (a high-cardinality `system.*`/`signoz_*` metric must be "reduce the label," never "drop the metric"). **Live-smoke status:** the SIG-070/COST-SIG reads are grounded on SigNoz's documented ClickHouse/Cost-Meter schema and our existing SIG discovery patterns but are **verify-pending a live SigNoz** — the benchmark SigNoz is on GKE and gcloud is de-authed this session, so a maintainer with live-SigNoz access should run the first live smoke (same honest posture as the PagerDuty verify-pending checks in 0.1.169). Read-only throughout; no mutation.
...
- **SNTRY-006 (source-map context) could never pass.** `/organizations/{org}/issues/{id}/events/latest/` returns the stack-frame flag as camelCase **`inApp`**, not snake_case `in_app` (confirmed live: `has_in_app_key=false`). The `select(.in_app == true)` matched zero frames, so the "every in-app frame has context" assertion failed closed on 100% of orgs regardless of whether source maps worked. Fixed to `select((.inApp // .in_app) == true)` (tolerant of both) — now returns a real verdict (live: matched a real in-app frame and correctly flagged one lacking context).
- **SNTRY-105 (inbound data filters) passed vacuously.** `filtered-transaction` is a default-on transaction sampler present on every modern project (confirmed live: `active=true` on all 6 projects), so the "any filter active" test passed everywhere. Fixed to **exclude `filtered-transaction`** from what credits a pass, and to gate the browser-only filters (`browser-extensions`, `legacy-browsers`) by project `platform` so a `python`/`go`/`node` backend is never flagged for a filter it cannot use. Live: frontend with `web-crawlers` on → pass; backend with only `filtered-transaction` → fail (platform-appropriate remediation).
- **SNTRY-015 (orphaned detectors) false-positived on built-ins.** Every workflow-engine org carries ~6 `error` and ~6 `issue_stream` detectors whose `workflowIds` is empty *by design* (confirmed live: 6 `error` detectors, all empty). Flagging every empty-`workflowIds` detector reported ~7 phantom orphans against the one real orphan. Fixed to **exclude types `error`/`issue_stream`** before the empty-`workflowIds` test and capture `config.mode` (3 = auto-detected) for remediation framing. Live: now lists only the single real `uptime_domain_failure` orphan.

**Verified:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 30/0, `plugin validate --strict`) + self-test **292/0/1** (new `layer_depth` regression locks for all three: camelCase `inApp`, the `filtered-transaction` exclusion, the built-in-detector exclusion). One new pressure scenario (checks-that-give-a-wrong-verdict-on-real-api-shapes). Every fix re-run read-only against the live org. (The ~30 proposed new checks and cross-provider/doctor-connect improvements from the same live-pass docs are a separate roadmap, not in this release.)

## 0.1.166

**audit-sentry alert-hygiene depth (5 new checks + a flap-mode extension), a setup-sentry mutation playbook, and one live-caught API bug fixed.** Distilled from a real incident-to-cleanup session on a live Sentry org (a chronically-open issue re-paging a channel unusable, environment-null rules whose names claimed an environment, never-fired dead-weight rules, an org with zero rule owners). The audit gains the missing *fire-history data source* and the *chronic-issue re-page join* that explain **why a channel is unusable**, not just which field is null — the strongest results a structural-only audit could never produce.

- **Five new read-only checks join the Alert rules and routing category** (SNTRY-106–110; the weight-25 category's denominator grows, it is not re-weighted):
  - **SNTRY-106 rule fire-history** — the shared data source. `lastTriggered` and `owner` are already on the rules list (free); the fire *count* is one GET per *already-flagged* rule against `/rules/{id}/stats/` (never a blanket estate fan-out). Feeds blast-radius evidence to SNTRY-011/014/101/102/107/109; not scored on its own.
  - **SNTRY-107 chronic-issue re-page join (flagship, high)** — joins long-open `ongoing` issues older than an age floor × un-gated notifying rules that still match them, and emits the *legal* re-page ceiling `matching_rules × 1440/frequency_min` pages/day for one permanently-open issue.
  - **SNTRY-108 name/scope/receiver coherence** — a "- Prod"/"- Dev"-named or PP-channel-routed rule whose actual `environment` is null or contradicts the claim is silent mis-scoping (distinct from SNTRY-102, which catches env-null alone).
  - **SNTRY-109 dead-weight rules** — never-fired (per SNTRY-106) **and** (snoozed **or** hair-trigger generic-keyword condition below the frequency floor); on, dead, deletable with no loss (distinct from SNTRY-016).
  - **SNTRY-110 ownerless rules** — `owner == null` across issue + metric rules; maintenance accountability (distinct from SNTRY-017 delivery routing).
- **SNTRY-103 extended** with two flap modes a non-null `resolveThreshold` + warning tier don't rule out: (a) a percent/failure-rate aggregate with no throughput floor (reads 100% on a 2-transaction overnight hour), and (b) warning + critical triggers resolving to the same channel (double-post).
- **setup-sentry gains `## Alert-rule remediation playbook`** (confirm-then-verify, under the existing change protocol): mandatory rule-set backup before the first mutation, GET-verify after every PUT/DELETE, the live `a metric-alert trigger cannot exist without an action` constraint (remove the trigger to silence a warning tier), tier-aware cadence (slow noisy tiers, keep New Issue/Critical/Escalating fast, **never age-gate regressions**), map rules to environments by receiver channel not by name (+ the no-env-tags-project SDK-first ordering), recorded revert conditions for temporary metric-query masks, and executor scope discipline. SNTRY-106–110 remediation routes here; runnable snippets added to `references/api-cookbook.md`.
- **Honest ceiling extended** for the new numbers: fire counts measure *emission, not annoyance* (no actionability rate without an incident feed); "never fired" is bounded by the ~90-day stats window ("not in the observable window", never "dead"); re-page ceilings are *legal maxima*, quoted alongside observed counts when `/stats/` data exists.
- **Live-caught bug (the reason this ships live-verified):** the SNTRY-107 issues query used `statsPeriod=90d`, which the issues endpoint rejects (400 — it accepts only `''`/`24h`/`14d`). Fixed to filter chronic issues server-side with the `age:+Nd` search token (grounded live: `age:+30d` = older than 30d), which also wires the previously-declared-but-unused `AGE_FLOOR_DAYS`. Every new read was then run read-only against a live Sentry org: `/issues/` returns `substatus`+`firstSeen`+`count`; `/rules/{id}/stats/` returns 2160 `{date,count}` buckets; metric `alert-rules` carry `timeWindow`/`comparisonDelta`/`detectionType`/`resolveThreshold`/`triggers[].label`; `owner`/`lastTriggered` read correctly.

**Verified:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 30/0, `plugin validate --strict`) + self-test **289/0/1** (new `layer_depth` locks for SNTRY-106–110, the scorecard denominator, the remediation route, and the setup-sentry playbook anchor). Two new pressure scenarios (fire-count-is-emission-not-actionability, chronic-issue-repages-a-channel-unusable). All smokes strictly read-only.

## 0.1.165

**Live-smoke wave 4 — audit-azure proven for the first time ever, and audit-cost's non-AWS phases proven; 5 cost-phase fixes.** A live Scoutflo-internal Azure subscription (the `kalpeshscoutflo` sponsorship tenant the azure work had been blocked on) and the re-authed GKE/SaaS estates let the last reachable surfaces be exercised read-only. **16 of 18 scored audits are now live-proven** (only pagerduty and jsm remain — both operator-credential-blocked, re-confirmed), plus audit-cost is now multi-phase live-proven.

- **audit-azure — first-ever live smoke, 0 bugs.** Ran read-only against the sponsorship subscription (scope-gated on the tenant first). Every documented api-version returned live **200 verbatim** — including the three checks that had never met a live tenant: **AZR-005** (`actionRules` 2021-08-08), **AZR-033** (`prometheusRuleGroups` 2023-03-01), and **AZR-042** (`diagnostic-settings subscription list`). Their verify-pending prose is reconciled to **read-path-confirmed / emit-path-unobserved** (the estate is a bare AKS-only dev subscription, so the actual emit conditions weren't exercised); **AZR-041** stays verify-pending (it needs a Log Analytics workspace, and this estate has none). The full emit → `check-findings.sh` (FINDINGS-OK) → `report.md` (`check-report.sh` REPORT-OK) → `report.html`/`inventory.json` pipeline ran clean.
- **audit-cost — Azure and Kubernetes phases now proven end-to-end** (were "authored, not-yet-live"); GCP/Datadog/DigitalOcean read paths verified live; `check-cost.sh` green with **$0 invented** (7 honest presence facts, every provider returned no native savings figure and none was fabricated). Five phase bugs fixed:
  - **datadog COST-DD-004** — `top_avg_metrics` needs a required `month=`/`day=` param (bare call = 400, always blocked) → adds `&month=$(date -u +%Y-%m)`.
  - **gcp COST-GCP-010** — `gcloud recommender recommenders list` is not a valid command (the emerging-recommender sweep silently found nothing every run) → replaced with an explicit sweep over the known cost recommender IDs via `recommender recommendations list`.
  - **kubernetes COST-K8S-007** — the reservation-ratio query counted Pending/unschedulable pods (`node=""`), producing a ratio > 1 and a negative surplus-node count → filters `node!=""` (scheduled pods only).
  - **datadog COST-DD-007** — `logs_by_index` is hourly; the jq emitted one row per hour → `group_by(index_name)` aggregates per index.
  - **digitalocean §10** — managed-DB slugs (`db-s-*`/`gd-*`) aren't in `doctl compute size list` (they're in `doctl databases options`) → the DB list-price context path corrected.
- **Cross-cutting durability (mechanisms, per "handle this better next time"):** added **3 pressure scenarios** for the dangerous false-green/false-fail *judgment* blind spots the live runs exposed (clickstack `webhookId`-less channel scored "wired"; gcp proto3-omitted `logCheckFailures` + `.host` nesting; lgtm vmsingle answering `/-/healthy` misclassified as Prometheus); closed the `maturity` schema/renderer inconsistency (renderer coalesces absent → `-`, schema marks it optional); and `integration-kit/SMOKE-MATRIX.md` now tracks 16/18 live-proven with each remaining blocker's exact unblock.

**Verified:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 30/0, `plugin validate --strict`) + self-test **281/0/1** (per-fix `layer_depth` locks for this wave). All smokes strictly read-only; Azure scope-gated on the tenant, GCP/AWS guardrails enforced. Remaining: pagerduty (fresh read-only REST key) and jsm (cloud id + API token) — both need an operator credential the plugin cannot self-provide.

## 0.1.164

**Live-smoke wave 3 — the 8 remaining reachable audits proven against real estates after cluster re-auth; 12 more real SKILL-vs-reality fixes.** With `gcloud auth login` restored, port-forwarded the live benchmark + elk-stack GKE observability stacks (read-only) and ran `audit-prometheus`, `audit-alertmanager`, `audit-clickstack`, `audit-signoz`, `audit-lgtm`, `audit-kubernetes`, `audit-gcp`, and `audit-grafana` against them. Every one built a `scoutflo-findings/v2` `findings.json` from real reads that passed `check-findings.sh` on the first emit and rendered — so **15 of 18 audits are now live-proven** (only pagerduty, jsm, azure remain, all operator-credential-blocked). Scores observed: prometheus 97 (Prom 3.14.0), alertmanager 75 (AM 0.34.0), clickstack 55 (CH 26.5 / HyperDX 2.35), signoz 33 (v0.138 — every documented endpoint/auth claim held verbatim), lgtm 57 (VM 1.142 / VLogs 1.50 / VTraces 0.8), kubernetes 8 (GKE 1.35, 0 bugs), gcp 60 (baseline matched), grafana 71 (10.4.1).

- **audit-prometheus** — PROM-023 queue-fill (`prometheus_notifications_queue_length / …_capacity`) returned **empty** on Prom 3.14 (the numerator carries an `alertmanager` label the denominator lacks, so the bare division fails vector matching and false-reads as healthy) → `sum(...) / max(...)`; PROM-040 `samples_dropped_total` doesn't exist on 3.14 → presence-guarded. Both **verified live on the spot**.
- **audit-alertmanager** — §8/§13 grouped `by (integration, receiver)`, but AM 0.34.0 emits `alertmanager_notifications_total{integration=...}` with **no `receiver` label** → `by (integration)`; and those queries hit Prometheus for AM self-metrics, which return empty when Prom doesn't scrape AM → documented the read-only `${AM_URL}/metrics` fallback (empty vector ≠ zero failures).
- **render-report-viz (all audits)** — the scorecard printed a literal `null` in the Maturity column when a category omitted `maturity` → coalesced to `-`.
- **audit-clickstack** — CS-040 receiver resolution: `GET /api/webhooks` needs a **required `?service=slack|generic|incidentio`** param on HyperDX v2.35 (bare call = 400), and the "wired" scan matched the keyword `webhook` — scoring a channel with **no `webhookId`** (routes to nobody) as a pass → now tests for a resolvable id.
- **audit-gcp** — GCP-010 read `.host` (false-FAILs every host; live nesting is `.monitoredResource.labels.host`) and GCP-016 read `.log_check_failures` (false-GREEN; real key is camelCase `logCheckFailures`, omitted by proto3 when false) → both now tolerate the raw and projected shapes.
- **audit-lgtm** — §1 metrics detection had no precedence (a live vmsingle answers `/-/healthy`, `/health`, and `/ready` at once, so "first match = Prometheus" misclassifies VM) → `vm_` self-metrics is now the decisive signal; §8 clarified that an empty Jaeger `/services` ≠ "no traces" (it only lists services with spans in the lookback — distinguishes ingestion-stopped from never-instrumented); §13.1 clarified vmalert `/flags` omits default-valued flags and redacts set values.
- **audit-signoz / audit-grafana** — no contract-breaking bugs (signoz's v0.138 endpoint/auth claims all held; grafana's commands worked verbatim); captured two doc-gaps: signoz's `clickhouse_sql` census lane + SIG-050 read-reach caveat, and grafana's VictoriaTraces/jaeger `/select/jaeger` proxy-path note. audit-kubernetes was **clean (0 bugs)** on GKE 1.35.

**Verified:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 30/0, `plugin validate --strict`) + self-test **277/0/1** (per-fix `layer_depth` locks for this wave). All smokes were strictly read-only (GET/list/describe + documented read-by-POST/session-login only); GCP/AWS guardrails enforced with explicit profile+identity verification. Estate findings surfaced for the team (not scored): the benchmark SigNoz OTel feed and the elk-stack VictoriaTraces ingestion both died in late August; 10 permanent synthetic `vector(1)` alerts from the decommissioned rbacwork exercise are still firing into vm-monitoring; a publicly-ingressed k8s-agent SA can list secrets cluster-wide with no namespace default-deny.

## 0.1.163

**Live-smoke wave 2 — audit-groundcover, audit-digitalocean, and audit-aws proven against their real estates; four more real SKILL-vs-reality fixes.** With this wave, **8 of the plugin's audits are live-proven on real estates under the v2 model** (sentry, datadog, zenduty, elk, groundcover, digitalocean, aws + the earlier sentry re-verify); each smoke ran strictly read-only, built a `scoutflo-findings/v2` `findings.json` from the real reads, and passed `check-findings.sh` + rendered (groundcover **92**/69 monitors — its first-ever live proof; digitalocean **56** with all 7 categories assessed live on 10 apps/62 alert rules/4 DBs/6 uptime checks; aws **29** with a 44-check ledger at 96% coverage).

- **audit-groundcover — `summary/query` carries more than the doc claimed (under-assessment fixed).** The live `POST /api/monitors/summary/query` response includes `noDataState` and `executionErrorState` (confirmed on a real 200), which the reference called YAML-only; the §4 projection dropped them, so on a `yq`-less machine GC-004/GC-005 were silently marked not-in-scope even though the data was already captured. The projection now retains both (plus `fullySilenced`), and the §4/§5 prose states the correct field split (only `autoResolve`/`customResolveThreshold`/`notificationSettings.*` are YAML-only). Also: GC-022's stuck-state filter matched the literal `"firing"`, which the live Grafana-style enum (`Normal`/observed sibling `Alerting`) never produces — a stuck monitor would have been missed; it now matches `alerting|firing` case-insensitively. The monitor-summary endpoint, previously "not confirmed in public docs", is now confirmed live; the GC-014 dedup and flagship-cascade honesty gates are live-proven to fire correctly (join fields were genuinely absent on the estate — reported as unreadable, never fabricated).
- **audit-digitalocean — DORT reads confirmed live; prose brought to live reality.** `DORT-001`/`DORT-002`'s reads ran against a real tenant (`active_deployment.phase`/`in_progress_deployment` present on all 10 apps): the banner now says **reads confirmed, emit-path unobserved** (the estate was healthy) instead of flat verify-pending. §8/§5 prose now uses the live policy type spellings (`v1/dbaas/alerts/<metric>_alerts`, not the shorthand `v1/dbaas/<metric>`); the executable jq was already correct. The 2026-07-20 logsink singular-path claim re-proven live.
- **audit-aws — DocDB double-count fixed (live-proven).** `aws docdb describe-db-instances/-clusters` is a façade over the shared RDS control plane and returns **all** RDS-family objects — live-confirmed: it returned the aurora-mysql clusters too. The estate-sizing block counted every Aurora object twice (skewing `estate.objects` and the small/medium/large path choice) and §4 wrote Aurora objects into the `docdb-*.json` evidence files. Both now filter `Engine == "docdb"`. Every other checked shape (null `DatapointsToAlarm` default, SNS subscription shapes, `list-addons`, event subscriptions, alarm-history window) matched the skill's assumptions.
- **Verified:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 30/0, `plugin validate --strict`) + local self-test **270/0/1**, including a new **`layer_v2`** that mechanically asserts every scored audit emits `scoutflo-findings/v2` with the v2 markers (and that `audit-cost` deliberately stays `scoutflo-cost/v1`) plus locks for each fix above. The remaining un-smoked audits are estate-blocked, not work-blocked: prometheus/alertmanager's configured estate has been decommissioned (they were live-verified at build time), and kubernetes/clickstack/signoz/lgtm-stores/grafana/gcp/pagerduty/jsm/azure need operator credentials or cluster re-auth (each with its exact unblock step recorded).

## 0.1.162

**Live-smoke of the v0.1.160 v2 migration against real estates — proved the emit and caught four real SKILL-vs-reality bugs the fixtures couldn't.** Ran `audit-datadog`, `audit-zenduty`, and `audit-elk` read-only against their live estates: all three built a `scoutflo-findings/v2` `findings.json` that passed `check-findings.sh` and rendered (datadog overall 45 / 38 monitors, zenduty 57, elk 78) — so the migration is now **live-proven**, not just fixture-validated, for these three. The live data then exposed:

- **audit-datadog — DD-016/DD-017 vendor corroboration used *guessed* `quality_issues[]` member strings.** The regexes tested for `alert_volume`/`stuck`, which match **zero** real monitors; the actual members (confirmed live on 36/38 monitors via `GET /api/v1/monitor/search`) are `broken_at_handle`, `missing_at_handle`, `alerted_too_long` (the real "stuck/alerting-too-long" signal), and `muted_duration_over_sixty_days`. Fixed the DD-016 corroboration regex to `broken_at_handle|missing_at_handle|alerted_too_long` and DD-017 to `alerted_too_long`, and updated the "verify-pending / only broken_at_handle proven" prose (references §6/§6.1/§7 + SKILL Phase 4/8) to the now-confirmed members. The load-bearing scoring was always the grounded handle-concentration / `overall_state` computations, so scores are unaffected — but the corroboration layer was silently dead.
- **audit-datadog — estate SLO count read the wrong path.** Estate-sizing read `.metadata.pagination.total_count`; the live `/api/v1/slo` returns the count at `.metadata.page.total_count`, so it under-reported (`slos=1` when the org had 3). Fixed to read `.metadata.page.total_count` first (old paths kept as fallback). Informational only (DD-032 reads the full list correctly).
- **audit-zenduty — ZD-031 mixed units (would false-fail every service).** `sla_object.acknowledge_time` is in **minutes** (live: `5` ack / `30` resolve), but the check compared it directly against the seconds-valued `mtta_seconds`. Taken literally, a healthy 218-second MTTA "missed" a `5` read as 5 seconds. Fixed references §8 + SKILL Phase 6 + the failure-mode row to multiply the SLA (and the `MTTA_TARGET_MIN` fallback, also minutes) by 60 before comparing.
- **audit-zenduty — ZD-030 overstated the per-service join.** Integration-created incidents return `service`/`service_ids` as `null` (live-confirmed), so the promised per-service escalation-policy attribution isn't always available; the aging count and `sla_object` still stand. Softened references §4 + SKILL Phase 6 to degrade to account/EP-level attribution and say so, rather than implying the join is always present.
- **audit-elk — clean (0 bugs).** Its collector correctly flattens the live Kibana 8.19.11 nested API shapes (`execution_status.status`, `schedule.interval`, `last_run.outcome`) into the fields the check commands expect; doctor's JSON-body assertion and the alerting-health check both passed against the real login-free API.

**Verified:** all four repo gates green (`leak-scan` CLEAN, `structure-check` 23, `run-tests` 30/0, `plugin validate --strict`) + self-test **229/0/1** (new `layer_depth` locks: DD-016/017 use the real `quality_issues` members and the dead `alert_volume` guess is gone; datadog SLO count uses `.metadata.page.total_count`; ZD-031 converts the minutes SLA ×60). Full real-estate live-smoke of the remaining migrated audits stays maintainer-pending only where the estate is tunnel-gated (prometheus, clickhouse) or the credential is stale (pagerduty, grafana, groundcover).
...
- **What v2 changes in each audit's emit (Phase 10):** a normalized `checks[]` ledger (one row per catalog check, `result ∈ pass/partial/fail/blocked/not-in-scope`, with a `reason` on every non-pass-non-fail row), `score.state` (`assessed`/`unassessed`), `score.scoring_model="assessed-only-v1"`, a `score.check_set` ledger fingerprint, `score.assessment` (applicable/assessed/scored/blocked/suppressed/not-in-scope + coverage %), per-category counts + `score = floor(((pass*2)+partial)*50/assessed)`, and `score.excluded[]` for out-of-scope categories that renormalize out. Every finding gains `scoring_scope` (`readiness`/`non-scored`) and unique `report_lanes` (`general-audit`/`ai-sre-readiness`); every report renders a `## Findings by purpose` section. The decisive difference from v1: **a denied or incomplete read is `blocked` (unassessed), never a silent `fail`** — so coverage is shown honestly instead of a confident zero. `audit-cost` deliberately stays on `scoutflo-cost/v1` (ranked $-savings, not a 0–100 score) — out of scope by design. No `toolkit.yaml` change; no finding-ID or weight changes.
- **Catalog ↔ scorecard category alignment (documentation consistency, not a scoring change).** Eleven audits' reference check-catalog "Category" column had drifted from their SKILL scorecard category names — kebab-case (`dashboard-semantics` vs `Dashboard semantics`), abbreviations (`Coverage/health` vs `Coverage and health`, `Log forwarding and account-level observability` vs `…retention…`), or a `Scope guardrail` label where the guardrail folds into a scored category. The catalog Category column is now byte-identical to the scorecard everywhere. **This is a doc-consistency fix, not a runtime one:** the audit emitter builds `checks[].category` from the scorecard, not the catalog (proven — aws and grafana have shipped and live-passed `check-findings.sh` for many releases *with* these mismatches), so it removes a latent trap and makes the catalog trustworthy reading, not a score change. Audits touched: prometheus, azure, clickstack, signoz, sentry, kubernetes, jsm, zenduty, groundcover (from the migration review) + aws and grafana (found by a follow-up mechanical aggregate check the review had missed). Non-scored parallel sections (Azure cost `AZROPT`, Kubernetes runtime `K8SRT`, DigitalOcean runtime `DORT`) and retired-check tombstones (`LGTM-005`) are intentionally left as-is.
- **Other audit-specific reconciliations:** `audit-azure` gains three scored catalog rows the scorecard's ID ranges already included but the catalog omitted (`AZR-033` AKS coverage, `AZR-041`/`AZR-042` Log Analytics coverage, verify-pending). `audit-datadog`'s Phase 3/4 body prose is reconciled to "live-verified (read-only)" for `DD-005`/`DD-016`/`DD-017` (matching sections 5.1/6.1 and Phase 8; only the narrow vendor `quality_issues[]` corroboration layer stays verify-pending). `audit-jsm`'s Outputs bullet now names the `## Findings by purpose` section. `audit-zenduty`'s Phase 3/4 and reference section headings are corrected to the full ID ranges (they had omitted `ZD-006`, `ZD-017`, `ZD-024`, making those checks look dropped).
- **Verified:** all four repo gates (`leak-scan`, `structure-check` — 23 checks, `run-tests` — 30 suites, `plugin validate --strict`) + the local self-test (215/0/1) green; an independent mechanical aggregate check confirms every scored catalog category matches its SKILL scorecard across all 17 catalog-bearing audits, with every scorecard category backed by ≥1 catalog row; a v2 schema structural sweep confirms all 18 scored audits emit `scoutflo-findings/v2` with `scoring_scope`/`report_lanes`/`check_set` and zero v1 residue. **Live-smoke:** a migrated audit (`audit-sentry`) was run against the real live Sentry org — its read-only probes returned real, auditable data (28 issue rules, `scrubIPAddresses=false`, 0 cron/uptime monitors), a v2 findings.json built from that data passed the strict `check-findings.sh` validator (overall reconciled, `check_set` matched) and rendered correctly through `render-report-viz.sh` (the `## Findings by purpose` lanes + scorecard viz), exercising the aligned category names end-to-end. An adversarial multi-agent review of the 14 migrations found the category-mismatch class (broader than the initial sample); a follow-up mechanical aggregate check caught two more canonical audits (aws, grafana) the review had missed. *Full real-estate live-smoke of the tunnel-gated benchmark endpoints (Prometheus, ClickHouse) and the credential-gated SaaS tenants (pagerduty, groundcover) remains for a maintainer with live access; those migrations are fixture-validated + fully gated.*

## 0.1.159

New **`alert-fatigue`** roll-up — release ④ of the metrics/alerting decomposition. A **non-scored, cross-audit** synthesis (sibling of the correlation engine and the cost roll-up) that runs inside `/scoutflo:audit-all` (Phase 3.6, right after correlation) and answers what no single backend can: across every tool you audited, where does the alerting noise concentrate, and does one incident page through several tools at once?

- **What it produces:** `${SCOUTFLO_AUDIT_DIR:-./scoutflo-audits}/alert-fatigue.json` (`scoutflo-alert-fatigue/v1`, `scoring_scope: non-scored`) with `AF-001` (alerting-noise concentration by source tool + severity), `AF-002` (**cross-source storm** — a service carrying alerting-noise findings from ≥2 tools, i.e. one incident paging through all of them), and `AF-003` (alert-to-incident ratio).
- **Zero provider calls, never re-scores.** It reads only this run's per-audit `findings.json` (same dual-glob + roll-up-dir skip as the correlation engine, so signoz/kubernetes/multi-target stacks are never dropped), selects the alerting-noise findings by `area`+title (so every audit's noise checks — ALR/LGTM/GRAF/DD/PD/JSM/ZD/GC — aggregate without a hardcoded ID list), and **cites** each source finding-ID. Every noise finding stays scored once in its home audit; this roll-up mutates nothing.
- **Never fabricates actionability.** `AF-003` is computed **only** from an operator-provided `fatigue.json` signal block (`{window, alerts_fired, incidents}`) — there is no incident feed in findings.json, so without that block the ratio is `not-in-scope` with the reason stated, never a made-up "N% actionable". `AF-001`/`AF-002` (which need no incident data) still report.
- **Wiring:** finding-ID prefix **`AF`** registered as a non-scored roll-up prefix; `alert-fatigue` added to the internal-helper allowlist and the start "behind the scenes" note; `audit-all` gains Phase 3.6 and renders the summary in its combined report (cost → 3.7, redaction → 3.8); `docs/CONTRACTS.md` C17 documents the contract; version bump. No `toolkit.yaml` change; the `fatigue.json` signal block is optional.
- **Grafana noise-once (release ⑤, folded in):** `audit-grafana` already scores its Grafana-managed alerting noise exactly once (Phase 4b, `GRAF-056`/`GRAF-100..103`, no reweight) and correctly bounds itself against the Alertmanager paging path (`audit-alertmanager`) and the Prometheus rule plane (`audit-prometheus`) — it needed no rework, so this completes the "noise scored once in each backend's home audit" principle with a one-line note that the `alert-fatigue` roll-up **cites** those GRAF findings but never re-scores them. That closes the Part-A metrics/alerting decomposition (① audit-prometheus → ② audit-lgtm narrowed → ③ audit-alertmanager → ④ alert-fatigue → ⑤ grafana noise-once).
- **Verified:** all four repo gates + self-test green; a 14-case hermetic test (`skills/alert-fatigue/tests/`) proves storm detection across two tools sharing a service, noise selection (excluding non-alerting findings), the non-scored/cites-source-IDs invariant, that no per-audit findings.json is mutated, and the `AF-003` signal-block ratio path. An adversarial review confirmed the invariants.
