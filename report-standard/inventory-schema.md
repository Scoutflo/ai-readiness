...
- **`items[]`** — one row per configured object the audit read:
  ...
  - **`kind`** (required) — a lower-snake type from the target's vocabulary: observability audits use `alert_rule`, `log_alert`, `activity_log_alert`, `alarm`, `alert_policy`, `alert`, `monitor`, `uptime_check`, `action_group`, `contact_point`, `notification_policy`, `connector`, `dashboard`, `datasource`, `receiver`, `silence`, `escalation_policy`, `schedule`, `team`; cloud audits use `vm`, `vmss`, `cluster`, `database`, `load_balancer`, `app_gateway`, `workspace`, `bucket`, `function`. Add a kind rather than overloading one. The alerting/monitor kinds that represent active coverage of `covers` — `monitor`, `alarm`, `alert`, `alert_rule`, `alert_policy`, `log_alert`, `activity_log_alert`, `uptime_check` — are the set the cross-tool coverage engine keys on (contract C14 in [../docs/CONTRACTS.md](../docs/CONTRACTS.md)); routing/muting kinds are not coverage.
  ...
  - **`env`** (optional) — the environment this object belongs to (`prod`, `pre-prod`, `staging`, `testing`, `dev`) when the provider states it or it is unambiguous. Omit it rather than guess. `render-report-viz.sh inventory` renders a **By-environment** paired view (per-environment counts + twin coverage) from this field when present, else derived from the object's name/`covers` (the matcher tests `pre-prod`/`pp` **before** `prod`); a platform-supplied value is normalized (`Production`→`prod`, `-pp`→`pre-prod`). This is a display grouping only — it does not enter `counts` or the score, so it never affects reconciliation.
