...
This renders two standalone reads over the same canonical findings: General audit
(reliability, security, capacity, backup, alerting, operational hygiene) and
AI SRE readiness (telemetry quality, service identity, topology, ownership,
change context, routing evidence, safe RCA/automation). Each view lists its
findings with severity, what's wrong, why it matters, and the recommended action;
a finding relevant to both appears in both. It does not create a second score or
change severity, and it does not repeat the detailed evidence — that stays in the
Findings section (or, for a non-scored finding, its own section).>
...
- **Findings by purpose** is derived from each v2 finding's `report_lanes` with `render-report-viz.sh lanes`. It is a per-audience read of the same findings — each view shows severity, what's wrong, why it matters, and the recommended action — not a second score. General audit covers foundational operational defects; AI SRE readiness includes only evidence-backed gaps that reduce telemetry correlation, incident context, RCA trust, or action safety. Do not invent a second score, change severity, or duplicate the detailed evidence (that stays in the Findings section).
