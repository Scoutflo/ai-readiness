...
- **Scoutflo Topology Readiness** follows [topology-readiness.md](topology-readiness.md): per critical service, are its identity, workload mapping, telemetry connections, connection details, tool identity, and match confidence sufficient for platform source and correlation sync once findings are fixed. Table headers use those plain-English check names (T-codes only in the legend line); confidence renders as `n/10`; any service below ready triggers the ticket-ready sync-readiness action plan table. Reads `topology-export.json`; a missing or target-mismatched export renders the matching state with its unlock path, never a bare "unavailable" and never a guess.
