...
The practical rule for a clean export: carry the service identity on a plain `service` or `service_name` attribute (not only inside a provider-specific field), alongside a `namespace` (or pod/container). A provider's own field can satisfy T4 while still not anchoring correlation, so emit the plain identity attribute too. Scoutflo maintains the current, exact attribute and naming requirements; check your export against Scoutflo's topology documentation before an import.
